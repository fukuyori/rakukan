...
use windows::{
    Win32::{
        ...
        UI::TextServices::{
            ITfCompartmentMgr,
            ITfLangBarItemButton, ITfLangBarItemMgr,
            ITfThreadMgr,
            GUID_COMPARTMENT_KEYBOARD_OPENCLOSE,
            TF_LANGBARITEMINFO, TF_LBI_STYLE_BTN_BUTTON,
        },
        UI::WindowsAndMessaging::{LoadImageW, HICON, IMAGE_ICON, LR_DEFAULTSIZE, LR_SHARED},
    },
};
...
pub unsafe fn langbar_add(thread_mgr: &ITfThreadMgr, item: &ITfLangBarItemButton) -> anyhow::Result<()> {
    thread_mgr
...
pub unsafe fn langbar_remove(thread_mgr: &ITfThreadMgr, item: &ITfLangBarItemButton) -> anyhow::Result<()> {
    let _ = thread_mgr
        .cast::<ITfLangBarItemMgr>()
        .map_err(|e| anyhow::anyhow!("cast ITfLangBarItemMgr: {e}"))?
        .RemoveItem(item);
...
pub unsafe fn set_open_close(thread_mgr: &ITfThreadMgr, tid: u32, open: bool) -> anyhow::Result<()> {
    let mgr = thread_mgr.cast::<ITfCompartmentMgr>()
...
pub fn get_open_close(thread_mgr: &ITfThreadMgr) -> bool {
    unsafe {
        let Ok(mgr) = thread_mgr.cast::<ITfCompartmentMgr>() else { return true; };
        let Ok(comp) = mgr.GetCompartment(&GUID_COMPARTMENT_KEYBOARD_OPENCLOSE) else { return true; };
        comp.GetValue().ok()
            .and_then(|v| i32::try_from(&v).ok())
            .map(|n| n != 0)
            .unwrap_or(true)
