...
/// ホストが復帰を試している間の文言（Issue #43 / #57）。
///
/// 推論の連続失敗（GPU ドライバの更新後など）や変換の詰まりで、ホストが
/// 自分を作り直している。黙って復帰させると利用者からは「たまに変換できない」
/// としか見えないので、一時的であることを伝える。原因は GPU とは限らないので
/// 文言では断定しない（検出した理由はホストのログとマーカーに残る）。
pub const BG_RECOVERING_STATUS: &str = "⚠ 変換エンジンを再起動中…";

/// 復帰できなかったときの文言（Issue #43 / #57）。利用者の次の一手を示す。
/// 原因は断定しない（上と同じ）。
pub const BG_UNRECOVERABLE_STATUS: &str =
    "⚠ 変換エンジンが復帰できません。Windows の再起動をお試しください";
