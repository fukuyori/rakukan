...
fn main() {
    let path = {
        let localappdata = std::env::var("LOCALAPPDATA")
            .expect("LOCALAPPDATA が設定されていません");
    ...
    println!("✅ サイズ: {} bytes ({:.1} MB)", meta.len(), meta.len() as f64 / 1_048_576.0);

    // 3. ファイルオープン
    let file = match std::fs::File::open(&path) {
        Ok(f) => { println!("✅ File::open 成功"); f }
        Err(e) => { println!("❌ File::open 失敗: {e}"); return; }
    };

    // 4. mmap
    let mmap = match unsafe { memmap2::Mmap::map(&file) } {
        Ok(m) => { println!("✅ mmap 成功: {} bytes", m.len()); m }
        Err(e) => { println!("❌ mmap 失敗: {e}"); return; }
    };
    ...
    println!("reading_heap_size={reading_heap_size}, entries_off={entries_off}, surface_heap_off={surface_heap_off}");

    if surface_heap_off > mmap.len() {
        println!("❌ ファイルサイズ不足 (mmap={}, needed={})", mmap.len(), surface_heap_off);
        return;
    }
    println!("✅ オフセット計算 OK");
