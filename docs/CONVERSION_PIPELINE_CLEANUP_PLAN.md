...
状態: 未実施。Phase 6b の `CandidateView` 導入後に着手する。
...
1. 表示可能な候補が既にあるケースでは `convert_sync` を呼ばない。
2. `convert_sync` を呼ぶ場合も、候補表を出した後の補完に寄せる。
3. どうしても候補表を出せないケースだけ、明示的な最終 fallback として残す。
