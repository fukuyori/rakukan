...
2026-09-08 の nick20002005 の回答（Issue #18）を併記。2026-09-12 時点の状態: **1 は確定**（下記）。2〜4 は推奨値で実装し、実機確認まで済んでいる（2 は 10-5、3 は 10-2、4 は 10-3。§9.6）。5 は「例外を入れない」で実装（10-2。テスト 1 件の差し替えで変更可。方式の判断は #35）。6 は「10-2 に含める」で実施済み。2〜4 について明示の確定は取っていないが、実装と確認の結果は上記のとおり。
...
| 10-1 | 入力ログの種別化と復元 | §3.4、F9 の接尾辞切り出し、性質テスト | 挙動変更なし（F6〜F10 の復元の修正のみ）。`hiragana_from_romaji_log` が detach していない状態で常に `hiragana_buf` と一致 | 実装済み（2026-09-12。engine 187 / tsf 99 件通過。実機確認済み 2026-09-12。コミット 2f71008） |
| 10-2 | 追い越し防止 | §3.3、pending 中の数字、`on_punctuate` LiveConv 分岐、TSF / engine の区切り判定の二重実装 | `k` + `.` = 「k。」、`n` + `.` = 「ん。」、`k` + `1` が `digit_width` に従う | 実装済み（2026-09-12。engine 202 / tsf 99 件通過。実機確認済み 2026-09-12（記号の位置の修正あり、§9.6）。コミット 2f71008 + 修正は未コミット） |
| 10-3 | Backspace 再生（pending あり） | §3.1、`force_preedit` 後の据え置き、`closes_run`、TSF テスト追加 | `kt` / `nt` / `tt` → BS → `a` が「か」「な」「た」 | 実装済み（2026-09-12。engine 218 / tsf 99 件通過。実機確認済み 2026-09-12。コミット 2f71008） |
| 10-4 | Backspace（pending なし）の log 書き換え | §3.2、2 段方式の具体化、F9 の回復、回帰テスト | `kya` → BS → F9 = `ki`、`sha` → BS → F9 = `shi`、`tta` → BS → F9 = `tt` | 実装済み（2026-09-12。engine 231 / tsf 99 件通過。実機確認済み 2026-09-12。コミット 2f71008） |
| 10-5 | Space 変換の未変換接尾辞 | §4.5 | 「たt」の `t` が候補と確定に残り、学習キーに入らない | 実装済み（2026-09-12。tsf 102 件通過。実機確認済み 2026-09-12（Backspace / Esc の復元の修正あり、§9.6）。コミット 93f8a75 + 修正は未コミット） |
...
### 9.6 実機確認の記録と追加修正（2026-09-12）

実機で確認した項目（すべて合格）:

| 段階 | 操作 | 結果 |
|---|---|---|
| 10-1 | 「るーむＡしつ」→ F9 → F6 | るーむＡしつ |
| 10-1 | `kon` → F9 → F6 | こん |
| 10-1 | 「1,000.0」→ F9 → F6 | １、０００。０（F6 の `to_hiragana` が ASCII を和文記号に変換するため、旧版と同じ表示。復元の効果はこの操作では観測できない。単体テストで固定） |
| 10-2 | `k` `.` / `n` `.` | k。 / ん。 |
| 10-2 | `k` Shift+`A` / `k` `1` / `n` `1` | kＡ / k1 / ん1（`digit_width` 半角） |
| 10-2 | ライブ変換中に `k` `.` | 今日はk。 |
| 10-3 | `kt` `nt` `tt` → BS → `a` | か / な / た |
| 10-3 | `kanakq` → BS → `a`、`kon` → Space → Esc → `k` → BS、F7 後の `k` → BS | かなか / こん / カ |
| 10-4 | `kya` `sha` `tta` → BS → F9 | ｋｉ / ｓｈｉ / ｔｔ |
| 10-4 | `wi` → BS → F9 → BS → `i`、`tta` → BS → `k` → BS | ｕ → 空 → い / っ |
| 10-5 | `tat` → Space / → `.` / → Enter / → Esc | 多ｔ / 多ｔ。/ 多ｔ 確定 / たt |
| 10-5 | `tat` → Space → BS / → Enter / → Esc → `a` | たt / たt 確定 / たた |
| 10-5 | `kyouhat` → Space → Enter | 今日はｔ |

実機で見つかり修正した 2 件（TSF、未コミット）:

1. **候補選択中の記号の位置**（`edit_ops.rs` `on_punctuate` の Selecting 分岐）: `tat` → Space → `.` が「た。ｔ」になった。表示を `候補 + 記号 + remainder` で組んでいたため、10-5 で remainder に置いた接尾辞の前に記号が入った。`remainder_reading` が空（remainder がリテラル）のときは打鍵順どおり `候補 + remainder + 記号` にし、読みも同じ順で組む。`remainder_reading` がある範囲指定変換は従来の順のまま
2. **Selecting からの復元（Backspace / Esc）**（`on_convert.rs` `restore_reading_from_selecting`）: `tat` → Space → Backspace で画面は「たｔ」なのに Enter が engine の「たt」を確定し、表示と確定がずれた。Backspace の分岐は engine に書き戻さず、Esc の分岐は `force_preedit` で接尾辞をリテラル化していた。両分岐を共通化し、engine に Space 前の状態（読み + 未確定ローマ字）が残っていれば engine の表示「たt」を使って書き戻さない（未確定の `t` が生きたまま戻り、続く `a` で「たた」）。範囲指定変換や区読点分割で engine が縮んでいる場合だけ `prefix + 読み + remainder` を書き戻す
