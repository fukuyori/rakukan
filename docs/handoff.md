...
| 項目 | 値 |
|---|---|
| main | `b1caf30`（2026-09-22 にローカル HEAD と GitHub の main が一致することを確認）。**作業ツリーに #55 の実装（`crates/rakukan-engine-rpc/src/client.rs`、`docs/DESIGN.md`、計画書の記録）が未コミットで残っている** |
...
| 3 | 状態の正しさに関わる修正（#55 / #50 / reload / #49 / config 読込失敗） | **#57 は PR #59 をマージ済み（2026-09-21）。実機未確認のため Issue は開いたまま、運用ログで確認する**。**#56 は 2026-09-22 に設計回答を受領し、Result 化の範囲・所有者不一致の扱い・BgTakeCandidates の訂正を返信済み。nick の設計反映後に実装範囲を確定する**（実装着手は未承認）。**config 読込失敗（J-5 の (1)）は #61 として完了**。**J-5 の (2) は調査・設計案を整理し、#65 / #66 を起票済み（2026-09-22）。方式の採用・実装着手は未承認、実機未検証**。**#55 は実装・単体テスト済み、実機で加算・抑止・回復を確認済み（2026-09-22、未コミット。30 秒経過後の再試行と、`Hello` / `Create` 各段階の失敗・Shutdown 直後の競合は実機未確認）**。#50 / #49 は未着手 |
