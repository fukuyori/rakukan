...
更新日: 2026-09-19（前版は main `c80dd04` 時点。履歴は git と [CHANGELOG.md](../CHANGELOG.md) を参照）
...
## 1. 現在の状態（2026-09-19、GitHub で確認）

| 項目 | 値 |
|---|---|
| main | `9061744`（2026-09-19 時点で origin と一致。作業ツリーの変更はこの記録の追記だけ） |
| 最新リリース | **0.11.7**（2026-09-14、タグ・GitHub Release あり） |
| 次のリリース番号 | **0.11.9**。0.11.8 は取り消して欠番（タグもリリースも作っていない） |
| engine ABI | main は 9（`crates/rakukan-engine/src/ffi.rs` と `crates/rakukan-engine-abi/src/lib.rs` の 2 か所）。**PR #59 が 10 へ上げる**（未マージ） |
| RPC protocol | 5（`crates/rakukan-engine-rpc/src/protocol.rs`）。#56 の実装で上げる見込み |
| `dict_schema` | 2 |
| 開いている PR | **#58（#53 の実装）/ #59（#57 の実装）**。どちらもレビュー済みで nick の回答待ち。下の「開いている PR と状態」を参照 |
...
| 段 | 内容 | 状態 |
|---|---|---|
| 1 | #53 の影響確認と方針回答、#54 のログ修正、#45 は PR 待ち | #54 はコミット済み・実ログ出力も確認済みで、**1 日分のログ量の確認だけ残る**。#53 は PR #58 が出てレビュー済み（L = 12 採用、記述の書き分け待ち）。#45 は PR 待ち |
| 2 | 0.11.9 リリースと 1 週間の再計測（Step 14） | 未着手 |
| 3 | 状態の正しさに関わる修正（#55 / #50 / reload / #49 / config 読込失敗） | **#57 は PR #59 が出てレビュー済み（確認事項 4 件の回答待ち）**。#56 は #57 の後。#55 / #50 / #49 / config 読込失敗は未着手 |
| 4 | 判断済みの機能（#35 本線 / #16）と nick の PR 受け入れ（#45 / #40-2 / #53） | #53 は PR #58 をレビュー済み。ほかは判断待ち・PR 待ち |

判断事項 J-1〜J-9 のうち、決定済みは J-1（#53。方式・PR の依頼・上限 L = 12 の採用まで）。J-2（#45 は依頼済みの PR を待つ）ほか J-3〜J-9 は、計画書の推奨のままレモンの判断待ち。

### 開いている PR と状態（2026-09-19）

| PR | 対象 | CI | 状態 |
|---|---|---|---|
| [#58](https://github.com/fukuyori/rakukan/pull/58) | #53（`参` / `拾` を数字保存の検証で数えない） | pass | **レビュー済み。L = 12 は採用。記述の書き分け待ち**（「TSV 上では根拠を失うのは 1 件」と「通常語フィルター適用後の件数は未確認」を分けて書く）。上限・非周期長文の比較計測は対応済み。実機ログはマージ後の発動確認に回す |
| [#59](https://github.com/fukuyori/rakukan/pull/59) | #57（詰まりの監視をホストへ移す） | pass | **レビュー済み。確認事項 4 件の回答・修正待ち**。(1) `acted` を probe の世代と組にする＋終了判断と probe 更新の同期（修正依頼）、(2) `on_stall_confirmed` の 50 ms sleep の目的、(3) ロード失敗時に古い probe が残る扱い、(4) `last_status` の遷移で回復処理が早期 return に遮られないか。**CI の成功とは別に、4 件を解消してからマージ判断に進む** |

詳細と根拠は [September_Late_Plan.md](September_Late_Plan.md) 9 節の 2026-09-18 / 2026-09-19 の各小節。
...
| 復帰マーカー | `%LOCALAPPDATA%\rakukan\engine-recovery.txt`（`<exited_at_ms> <attempt>`、#43）。**PR #59 で理由欄が付いて 3 列になる**（旧形式は「不明」として読む） |
...
| Issue | 内容 | 状態 |
|---|---|---|
| #62 | MOZC 辞書を master から取得していて、生成元のリビジョンを固定も記録もしていない | **実装済み・未コミット**。取得元を完全な SHA `cbbb6e1bd181cb9f3b409622d916a53a35400ec7` で固定（2026-09-13、リポジトリ全体の master 先頭。必須 12 ファイルの存在と raw URL を確認済み、[確認結果](https://github.com/fukuyori/rakukan/issues/62#issuecomment-5740309247)）。`build.json` に `mozc_rev` / `mozc_source` を記録し、SHA 不一致・`mozc_rev` 欠落で自動的に作り直す（`dict_schema` 不一致と `RAKUKAN_FORCE_DICT=1` の既存条件は維持）。取得済み TSV は SHA ごとの作業フォルダに置き、別リビジョンのものを再利用しない。必須入力が 1 つでも欠けたら再生成を中止し、既存の辞書と `build.json` を残す。生成は別名で行い、`scripts/dictionary-swap.ps1` が旧ペアのコピーと永続的な状態記録で差し替える。確定前は旧状態を復元し、確定後は新ペアを保持して掃除だけ再開する。復元失敗時はバックアップと状態記録を保持して停止する。`rakukan-dict-builder` の doc コメントの列の並びの誤りも修正。ビルダーのメタデータ単体テスト 5 件 + 再生成判定 5 通り + 差し替え・中断復旧テスト 14 ケース。**SHA 固定が保証するのは取得元データの一致で、辞書のバイト単位の一致は別の検証事項** |
| #61 | config.toml のパースに失敗すると、警告なしで全設定が既定値に戻り、その既定値がホストへ送られる | **実装済み・未コミット**。`init_config_manager` / `ConfigManager::new` の `unwrap_or_default()` が原因。`engine_reload` のスレッドは `init_config_manager()` の直後に `build_engine_config_json()` を呼ぶので、既定値がホストへ送られる（**推測ではなくコードで確認**）。修正: 再初期化は失敗時に直前の設定を保持し、初回だけ既定値を使う。どちらもパスとエラー内容を警告に残す。**ファイル不在も同じ扱い**（削除を暗黙の設定リセットにしない）で、再初期化から `config_save_default()` は呼ばない。単体テスト 10 件。計画書の J-5 の (1) に対応。J-5 の (2)（reload 通知の全プロセス反映）は未着手 |
| #60 | TSF のログがローテーション後も旧世代へ書き込まれ、16 MiB を超えて増え続ける | **実装済み・コミット済み**（`e37c645`。**実機確認は未実施**）。原因は「開いたハンドルが rename に追随する」「サイズ判定が `DllMain` だけ」の 2 つ。ログを起動インスタンスごとの `rakukan-tsf-<PID>-<起動識別子>.log` に分け、サイズ確認・退避・開き直しを書き込み処理の中（`Mutex<RotatingLog>`）へ移した。掃除は起動インスタンスごとのロックを排他で取れたときだけ行い（PID の生存では判断しない）、保持目安は 256 MiB（使用中は消さないので厳密な上限ではない）。旧方式の `rakukan.log` は自動削除の対象外。統合用に `scripts/merge-logs.ps1` を追加。ログ関連単体テスト 18 件。退避途中の失敗は `.tmp` と進行状態を保持して同じ操作から再開し、既存の退避先を上書きしない。`.1`〜`.5` の共有違反・最後の `.tmp` 移動失敗・進行状態のない `.tmp` の保護を回帰テストで確認 |
| #57 | 長文を Space で変換すると、返っているのにエンジンが再起動されることがある（`bg_timeout_watchdog` の詰まり開始時刻が残る） | **確認済み・設計への意見を求めて返信済み**（2026-09-17、nick。[返信](https://github.com/fukuyori/rakukan/issues/57#issuecomment-5709038255)）。`candidate_window.rs` の `on_waiting_timer` が `bg=done` で `bg_timeout_watchdog(false)` を呼ばないため、古い時刻から数えて閾値を超え `engine_reload_force()` が走る、という報告。2026-09-16 の `rakukan.log` に `conv worker stuck 3618s` / `65s` の実例。手元の修正はフォークの `e8eeac9`（#56 と同じコミット。PR にするなら分けると書かれている）。**主経路はコードと合う**。待機タイマーの上限（10 秒）が監視の閾値（20 秒）より短く、今の詰まりの検出は記録を次の待機まで残すことで成り立っているので、UI の待機の終了で記録を消すと検出が効かなくなる。一方で、観測していない間に実行が完了して別の実行が始まることや、監視している実行・エンジンが置き換わることもあるので、TSF 側で `false` を足す案では足りない。**判断: 監視対象を「同じ実行中の変換」にし、B（エンジン側で実行番号と開始からの経過時間を持ち、ホストが監視する）を軸に設計する**（計画書 3-6）。**実装前の判断 5 点を 2026-09-18 に返信し（[コメント](https://github.com/fukuyori/rakukan/issues/57#issuecomment-5725464861)）、PR #59 が出てレビュー済み**（確認事項 4 件の回答待ち）。閾値 30 秒は「正当な処理でも超えうる誤発動を許容する暫定値」として採用（`GEN_TIMEOUT_SECS` は生成 1 回の上限で、変換 1 回は かな run ごとに生成を呼ぶため k × 15 秒かかりうる）。ABI は 9 → 10 |
| #56 | 未確定文字があるときにホストが再起動すると、Space も Backspace も効かなくなる | **確認済み・設計への意見を求めて返信済み**（2026-09-17、nick。[返信](https://github.com/fukuyori/rakukan/issues/56#issuecomment-5709038552)）。新しいホストの読みバッファが空になり、TSF のセッションと composition だけが未確定文字を持つ、という報告。`handle_action: "Input" state=Preedit("…") bg=idle hira=""` の実例。手元の修正はフォークの `e8eeac9`（composition が生きていてエンジンの読みが空なら、セッション側の読みを `force_preedit` で戻す。Esc 直後は対象外）。**前提（新しいエンジンは読みが空、TSF は合わせない）はコードと合う**。ただし、ホストやエンジンの世代を示す情報が無く、RPC 失敗も「空」になる。エンジンは全アプリで共有。通常入力ではセッションが `Idle` のままで復元の元が無い。`force_preedit` は未確定ローマ字を失う。自動再送で読みを変える要求が二重に適用されうる。**判断: エンジンの世代情報を追加する方向で設計する**。世代だけでは足りず、共有エンジンの所有者（どの composition の読みか）、復元元の編集状態の保持場所（TSF 側）、世代違いを適用前に検出する順序、再送の二重適用も合わせて設計する。未確定ローマ字・打鍵履歴の復元は未確定（計画書 3-7、9 節「#56 / #57 の確認」「#56 / #57 の設計」）。プロトコルの版を上げる見込み。**2026-09-18 にコード照合の結果を返信し、2 度の訂正と回収規則の確認事項を追加した**（[1](https://github.com/fukuyori/rakukan/issues/56#issuecomment-5725537904) / [2](https://github.com/fukuyori/rakukan/issues/56#issuecomment-5725557345) / [3](https://github.com/fukuyori/rakukan/issues/56#issuecomment-5725568117)）。要点: 識別子はプロセス再起動でも衝突しない形にし、世代は要求ごとに照合する／**復元用 RPC を 1 つ設ける**（既存のバッチでは不可分にできない）／判定の優先順は「世代 → 要求番号 → 所有者」で、**所有権の移動では適用済み記録を破棄しない**（破棄すると `Learn` の二重適用が残る）／`input_log` は復元せず、失われるのは「復元**前**の打鍵履歴に依存する機能」。**#57（PR #59）の後に進める。実装着手は未承認** |
...
| #53 | 数字と「参考」を同じ変換単位で打つと候補が捨てられる（`参` `拾` を大字 1 文字として数える） | **PR #58 が出てレビュー済み**（[依頼コメント](https://github.com/fukuyori/rakukan/issues/53#issuecomment-5708502866)）。方式: 候補の `参` / `拾` が、入力の読みに対応する辞書の語の一部と確かめられたときだけ数えない（辞書による語と読みの対応付け + 動的計画法による存在判定）。**照合表で引く読みの長さに上限 `MAX_READING_CHARS = 12` を入れ、上限超の語は従来の検証に戻す**（非周期 120 文字で 3.5 ms → 0.81 ms、`lookups` 7,179 → 1,293）。残るのは記述の書き分けだけ。経緯・設計・受入条件・既知の制限は計画書 9 節の「#53」で始まる小節 |
...
| #58 の `digits::license:` ログ | マージ後、インストールして `rakukan-engine-dll.log` で発動を確認する（マージ条件からは外した） |
| #59 の詰まりの検出 | 実機未確認。詰まりを意図的に起こす手段（#43 の `force_inference_failure` に相当する診断設定）は PR #59 に入っていない |
| #59 の `last_status` が `"done"` のまま回復処理を素通りする経路 | コードを追った結果で、実機では確かめていない（計画書 9 節 2026-09-19 の #59 レビュー 4） |
