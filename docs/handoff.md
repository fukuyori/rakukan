...
更新日: 2026-09-22（#65 の実装・範囲付きの実機結果・残る試験と実施順序を反映。履歴は git と [CHANGELOG.md](../CHANGELOG.md) を参照）
...
| 項目 | 値 |
|---|---|
| ローカル main | `8f67a6a`（2026-09-22。#65 の方式採用と詳細設計の記録まで）。#65 の実装・検証記録と本引き継ぎの更新は作業ツリーに未コミット。GitHub 側の main・CI は今回再確認していない |
...
| J-5 の (2) | [#65](https://github.com/fukuyori/rakukan/issues/65) は方式採用済み、実装・自動テスト済みで未コミット。**通常操作 4 項目は範囲付きで実機確認済み**。故障・競合試験と厳密な同一 mtime の検出確認が残り、再接続への組込みは #56 と調整待ち。[#66](https://github.com/fukuyori/rakukan/issues/66) は方式採用・実装着手が未承認 |
...
- **#65 は方式採用・詳細設計を経て実装・自動テスト済み、未コミット。#66 は方式採用・実装着手が未承認**。詳細は [September_Late_Plan.md](September_Late_Plan.md) 9 節の J-5 の (2) と「2026-09-22 #65 の実装」を参照
...
| Issue | 内容 | 状態 |
|---|---|---|
| [#65](https://github.com/fukuyori/rakukan/issues/65) | 設定変更を全 TSF プロセスへ反映する（通知漏れ・同一 mtime の変更に対応） | **方式採用済み、実装・自動テスト済みで未コミット。通常操作 4 項目の実機確認は範囲付き**。厳密な同一 mtime、通知遮断、監視失敗・復旧、故障・競合の処理経路の確認が残る。次は試験設計。プロトコル変更なし。**再接続への組込みは #56 と調整待ち**。巻き戻り防止は #66 と分担。詳細は計画書 9 節「2026-09-22 #65 の実装」 |
