...
更新日: 2026-09-19（前版は main `9061744` 時点。履歴は git と [CHANGELOG.md](../CHANGELOG.md) を参照）
...
| 項目 | 値 |
|---|---|
| main | `fbe7848`（2026-09-19 時点で origin と一致。作業ツリーの変更はこの記録の追記だけ） |
...
| 3 | 状態の正しさに関わる修正（#55 / #50 / reload / #49 / config 読込失敗） | **#57 は PR #59 が出てレビュー済み（確認事項 4 件の回答待ち）**。#56 は #57 の後。**config 読込失敗（J-5 の (1)）は #61 として実装・コミット済み**。#55 / #50 / #49 と、reload 通知の全プロセス反映（J-5 の (2)、**次に着手予定**）は未着手 |
| 4 | 判断済みの機能（#35 本線 / #16）と nick の PR 受け入れ（#45 / #40-2 / #53） | #53 は PR #58 をレビュー済み。ほかは判断待ち・PR 待ち |

判断事項 J-1〜J-9 のうち、決定済みは J-1（#53。方式・PR の依頼・上限 L = 12 の採用まで）と **J-5**（2 件とも起票する。(1) は #61 として実装・コミット済み、(2) が次の着手予定）。J-2（#45 は依頼済みの PR を待つ）ほかは、計画書の推奨のままレモンの判断待ち。
