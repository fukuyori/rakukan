...
更新日: 2026-09-22（#56 の設計回答と返信、J-5 の (2) の調査・設計案と #65 / #66 の起票を記録。履歴は git と [CHANGELOG.md](../CHANGELOG.md) を参照）
...
| 項目 | 値 |
|---|---|
| main | `b40b428`（2026-09-22。#55 の実装と記録をコミット・プッシュ済み。CI は実行中の状態で確認を終えた） |
...
| J-5 の (2) | 調査・設計案を整理し、2026-09-22 に [#65](https://github.com/fukuyori/rakukan/issues/65)（全プロセスへの反映）と [#66](https://github.com/fukuyori/rakukan/issues/66)（古い設定の巻き戻り防止）を起票済み。**方式の採用・実装着手は未承認、実機未検証** |
...
- [#65](https://github.com/fukuyori/rakukan/issues/65) 設定変更を全 TSF プロセスへ反映する（2026-09-22）。J-5 の (2) の A。ディレクトリ変更通知・定期確認・本文比較と、正常設定の組の保持、検出とエンジンへの反映待ちの分離を扱う。**プロトコル変更なしの案。背景監視から再起動を増やさず、古い設定の再送を防ぎ切る保証は #66 で扱う**
...
- **#65 / #66 は起票と相互リンクの更新まで完了。方式の採用・実装着手は未承認、実機未検証**。詳細は [September_Late_Plan.md](September_Late_Plan.md) 9 節「2026-09-22 J-5 の (2) 設定の全プロセス反映と古い設定の再送防止: 調査と設計（案）」と末尾の起票記録を参照
...
| Issue | 内容 | 状態 |
|---|---|---|
| [#65](https://github.com/fukuyori/rakukan/issues/65) | 設定変更を全 TSF プロセスへ反映する（通知漏れ・同一 mtime の変更に対応） | **2026-09-22 起票済み・設計案あり。方式採用・実装着手は未承認、実機未検証**。プロトコル変更なしの案。#61 の正常設定保持を引き継ぎ、読み取りの組と反映待ちを管理する。巻き戻り防止は #66 と分担。詳細は計画書 9 節の J-5 の (2) |
...
- **config の全プロセス反映は未解決**: auto-reset の reload イベントを連続で Set しても、全プロセスが受け取る保証にはならない。変更検出は #65、古い設定の再送による巻き戻り防止は #66 で扱う（設計案・実装未着手）
