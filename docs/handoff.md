...
| Issue | 内容 | 状態 |
|---|---|---|
| #60 | TSF のログがローテーション後も旧世代へ書き込まれ、16 MiB を超えて増え続ける | **実装済み・未コミット**。原因は「開いたハンドルが rename に追随する」「サイズ判定が `DllMain` だけ」の 2 つ。ログを起動インスタンスごとの `rakukan-tsf-<PID>-<起動識別子>.log` に分け、サイズ確認・退避・開き直しを書き込み処理の中（`Mutex<RotatingLog>`）へ移した。掃除は起動インスタンスごとのロックを排他で取れたときだけ行い（PID の生存では判断しない）、保持目安は 256 MiB（使用中は消さないので厳密な上限ではない）。旧方式の `rakukan.log` は自動削除の対象外。統合用に `scripts/merge-logs.ps1` を追加。ログ関連単体テスト 18 件。退避途中の失敗は `.tmp` と進行状態を保持して同じ操作から再開し、既存の退避先を上書きしない。`.1`〜`.5` の共有違反・最後の `.tmp` 移動失敗・進行状態のない `.tmp` の保護を回帰テストで確認 |
