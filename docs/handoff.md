...
| #50 | 破棄済み DocumentManager のポインタが `dm_modes` に入り直し、アドレス再利用で前のモードが復元される | **J-4 は (a)（世代と生存状態）に決定（2026-09-24）。詳細設計（案）を計画書 9 節「2026-09-24 #50 …」に記録**: TSF スレッドの thread-local な台帳で世代の登録（`OnInitDocumentMgr` / 初回目撃）と失効（`OnUninitDocumentMgr`）、`FocusChange` にキュー投入時の `DmRef { ptr, gen }`、遅延処理で dead / 世代違いの `prev` は保存せず `next` は復元も apply もしない、`ModeStore` の鍵を `DmRef` に、`doc_mode_remember_current` も生存確認、#51 の `base_dm` も `DmRef`。4 点（台帳の thread-local、dead な next は復元も apply もしない、`base_dm` も `DmRef`、死んだ slot の上限 1024）は推奨どおりに決定（2026-09-24）。**実装着手は未指示** |
