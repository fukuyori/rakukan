...
6. 接続試行（接続 → spawn → `Hello` → `Create`）が最終的に失敗すると、どの段階の失敗でも
   1 試行につき 1 回数え、15 秒に 3 回でその TSF プロセスからの spawn を 30 秒止める
   （`HostSpawnGuard`、Issue #55）。spawn に失敗しても別プロセスが起動したホストへ
   `Hello` / `Create` まで成功すれば数えず、回数と抑止を消す。抑止中の試行はどの段階の失敗も
   数えず期限も延ばさない（既存ホストへの接続は試み、成功すれば抑止も解ける）。
   起動後の異常終了の繰り返しは数えていない（#43 の復帰マーカーの範囲）
...
- **閾値の超過を確認した時点で復帰を決定する。** 確認の直後にその変換が完了しても、直後に始まった
  別の実行が巻き添えになっても、設計どおりの挙動として扱う
- 口は**世代**を持ち、持ち替え・取り外しのたびに 1 つ進む。監視スレッドの「この実行は処理済み」の
  記憶は (世代, 実行番号) の組で持つ（DLL の variant が変わると実行番号が 0 から数え直しになるため、
  番号だけだと詰まりを見逃す）。確定は口のロックを保持したまま行い、そこで世代を照合するので、
  **古い DLL の観測で現在のホストを終了させることはない**
- エンジンを外すとき（`Reload`）は口も外す。ロードに失敗したときに、停止しきっていない古い DLL の
  ワーカーの `Running` を根拠に終了しないため
- 詰まりを観測したら `last_status` に番兵を入れ、次の観測が早期 return で落ちないようにする
  （`observe` は同じ状態なら数えないが、詰まりは `observe` を通らないため）
...
### インストール時の辞書差し替え（Issue #62）

`scripts/dictionary-swap.ps1` が `rakukan.dict` と `rakukan.dict.build.json` を
組として更新する。生成は `.new` へ行い、差し替え前に旧ファイルを `.bak` へコピーする。
`rakukan.dict.swap.json` は一時ファイルへの書き込み・flush 後に置換し、処理段階と
旧ファイルそれぞれの有無を記録する。

| 記録された段階 | 次回の復旧処理 |
|---|---|
| `preparing` | 本体は未変更。途中まで作ったバックアップ等を掃除する |
| `prepared` | バックアップをコピーして旧状態へ復元する。旧ファイルが無かった場合は新ファイルを除去する |
| `committed` | 新ペアを保持し、残ったバックアップ等の掃除だけを再開する |
| `rolled_back` | 復元済みの旧状態を保持し、掃除だけを再開する |

復元はバックアップを消費せず、両ファイルの復元完了を記録してから掃除する。
状態記録は最後に削除するため、バックアップ片方の削除後に中断しても新旧を混ぜない。
復元・掃除の失敗、壊れた状態記録、状態記録のない `.bak` は処理を停止して残す。
未解決のバックアップを新しい差し替えで削除しない。
状態記録に旧ファイルの有無を残すので、初回インストールやメタデータのない旧辞書も復元できる。

`scripts/test-dictionary-swap.ps1` はリポジトリの `target` 配下だけで 14 ケースを検証する。
共有違反による復元失敗と再実行、各段階の中断状態、確定後の掃除失敗を含む。
これはプロセス中断とファイル操作失敗の検証であり、実機インストールや電源断の耐久試験ではない。
