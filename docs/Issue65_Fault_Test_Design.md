...
- （2026-09-24 に追記）個別の監視源失敗、設定アプリの保存失敗、同一 mtime・サイズの変更の待機ループでの取得、対象アプリの故障制御、通常版への復元は、下の「起動スクリプトと実機の初回確認」以降でいずれも実機確認した。#65 の完了判定は、再接続への組込み（#56 と調整待ち）が別残件のため保留。
...
**設定アプリの保存失敗（2026-09-24 14:42〜14:44 JST、レモンの操作）**: `config.toml` に読み取り専用属性を付けて（SHA256 `6301DBD3…`、候補文字サイズ 16）、設定アプリで 16 → 32 に変えて保存。「設定を保存できませんでした / Access to the path is denied.」のダイアログが出て「保存しました」は出ない。保存後も SHA256 と値は不変、`config.toml.*.tmp` は残らず、TSF ログに `reload event received` は無い（一時ファイル作成のディレクトリ通知による `read (notify) -> Unchanged` が 1 プロセスに出るだけ）。属性を外して再保存すると成功し（05:44:20 UTC、全プロセスが `config published` rev+1、`engine_json_changed=false`、イベントを受けた 1 プロセスは `no pending config change`）、16 に戻す保存も同様（05:44:35）。終了時の SHA256 は開始時と同じ `6301DBD3…`。

これで実機の試験ケースはすべて実施した。残るのは #56 との調整待ちの再接続への組込みと、#66 の古い設定の採用拒否で、いずれも #65 の故障試験の範囲外。
