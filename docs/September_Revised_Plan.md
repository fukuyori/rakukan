...
- 追加で分かった問題点: 学習履歴を個別に削除する手段が無い（#37 に記載。実機確認では `learn_history.bin` を退避して対応）。
...
| 2 | Step 12 学習順位と学習履歴 v2 | #37、#13、#42 | **完了（2026-09-12、未リリース）**。#13 / #42 クローズ。#37 は学習履歴の削除手段（8）の扱いを決めてクローズ。#29 は保留 |
| 3 | Step 13 低頻度改善と計測 | #39、#43 | 再試行の安全条件（キースレッド非ブロック、retry storm 防止）と、推論失敗からの復帰設計（#43。host 側での検知と自己復帰、TSF は状態問い合わせと表示だけ）を合わせて設計してから |
| 4 | Step 14 リリースと再計測 | なし | 対応するステップの実機確認後 |
| 5 | 10 月改修: インストール・更新・削除の再設計 | #33（PR #30 の方式を採用） | `docs/October_Install_Plan.md` の Step 1〜6。着手は Step 10〜13 の後 |
