...
対象: R-4 / PR #7 / Issue #6。残件（かな run の辞書参照）は Issue #16
...
対象: G-5（P1。Step 8–9 と同じ engine DLL の変更としてまとめる）。Issue #34（問題点の記録）、#38（英単語の破壊。PR #26）、#35（`z` 系キー列の扱い）。詳細設計は `docs/Step10_Romaji_Rebuild_Plan.md`
...
対象: R-5 / G-4 / PR #12 / Issue #13。未整理の問題点は Issue #37。完了後に #29（`priority = "low"`）を判断
...
### 0.11.4 リリース・IME オン/オフ統一・Issue 分割（2026-09-07〜09）

- **IME オン/オフへの状態統一（`df58571`、0.11.4）**: 入力モード 3 値（ひらがな / カタカナ / 英数）を廃止し `ImeMode { On, Off }` に統一。カタカナモードは廃止（`mode_katakana` は `katakana` = F7 変換に読み替え）。切替経路を `tsf/ime_sync::apply` と `switch_ime` に集約し、内部状態を唯一の正としてコンパートメントは導出して書くだけにした。言語バーメニューで英数にした後にショートカットで切り替えると表示が「A」のまま残る不具合の修正。keymap の旧アクション名（`mode_hiragana` / `mode_alphanumeric`）と `default_mode` の旧値は別名で互換維持。修飾キーの左右指定（`LCtrl` / `RAlt` 等）、設定アプリの主要ショートカット再構成（IME ON / IME OFF 追加）とキー名ヘルプ、設定アプリの単一インスタンス化を同梱。PR #24 は同じ処理が `switch_ime` に入ったため取り込み済みとしてクローズ。
- **Issue の分割（2026-09-09）**: Issue #18（棚卸し）の各項目と計画書だけにあった課題を、現状の問題点として独立 Issue にした。#32 文節変換（A）、#33 インストーラー再設計（10 月計画）、#34 ローマ字入力の再構築（Step 10、B / F）、#35 区読点だけの読みとリーダー記号（G）、#36 新しい入力欄の IME 状態（H の残り）、#37 学習順位と履歴 v2（Step 12、E）、#38 英単語の破壊（F、PR #26）、#39 初回変換の再試行と Convert 計測（Step 13）、#40 候補・予測の品質は再現確認が先（D）。#18 には分割一覧を投稿し、棚卸しの記録として残す。
- **方針の更新**: #18 の 9/4 返信で示した A / G / H の条件は、その後の検討で方針が変わっている。A（文節変換）は #32 に切り出した（`docs/Segment_Edit_Plan.md`）。G は打鍵中の自動畳み込みを採らない方向で #35 と `docs/Symbol_Leader_Input_Plan.md` で検討中（PR #31 は保留）。H は #36 で可否を判断する。9/4 の返信自体は訂正せず、PR が来た時点で個別に判断する。

### Step 10 完了・0.11.5・PR #41 / #44 の対応（2026-09-10〜12）

- **Step 10（#34）**: `docs/Step10_Romaji_Rebuild_Plan.md` §9 の 5 段階で実装。10-1 入力ログの種別化（`InputEntry` / `output` 連結による復元）、10-2 追い越し防止（`close_pending`）、10-3 pending ありの Backspace 再生、10-4 pending なしの Backspace の log 書き換え（接頭辞の再生 → 逆引き。母音は母音字）は `2f71008`、10-5 Space 変換の接尾辞は `93f8a75`、実機で見つけた 2 件（候補選択中の記号の位置、Selecting からの復元）は `8e9e2c6`。実機確認 13 項目すべて合格。§5 の判断は 1 を A・a′・i で確定、2〜4 は推奨値で実装、5 は #35 待ち（例外なし）。#34 クローズ。
- **0.11.5（`fe46e09`、タグ作成・プッシュ済み）**: Step 10 の 5 件と PR #41 の固まり解消を Fixed に記載。タグ 0.11.4 は未作成のまま。
- **PR #41（推論失敗で Space が固まる）**: 固まりの解消だけ取り込んだ（`b657bb4` engine の "error" 状態は cherry-pick、`909ecc4` TSF は 10-5 後の経路に合わせて書き直し）。host の自動再起動は責務分担（GPU リソースは host が管理）と再起動ストーム（TSF プロセスごとのクールダウン）の懸念から見送り、復帰の設計は #43 に起票して Step 13 と合流させる。PR はクローズ。
- **PR #44（#38 英単語の破壊、#26 の出し直し）**: `input_log` の `output` 累積を境界に使う実装。Codex P1（助詞を含まない日本語の前置きが英単語に巻き込まれる）は再現したが、誤発動は素の ASCII が残る読みに限られ正常な日本語を壊さないため、既知の限界として受け入れて rebase マージ（`df784ce`）。限界の記録とテストを `4d055ac` で追加。実機確認 3 件合格、#38 クローズ。
- **#40（候補・予測の品質）**: nick の確認で 1・3 は main で再現せず、7（速く打つとプレビューが更新されない）は main でも起こる → #45 に切り出して PR を依頼。2 は「候補末尾に文字種候補を添える」縮小版を Step 12 の順序入れ替え後に PR として受ける、4 は見送りで合意。

### Step 12（2026-09-12）

- 12-0 調査: `store.rs` は v2 形式・v1 破棄・数値検証が既に main にあり、#37 の 2・3 は着手時点で済んでいた。減衰（PR #12）は独立レビューで問題なし。`rakukan.dict` の実測で通常語の cost 最大は 18,318、記号（3000 固定）3,527 件、絵文字（6000 固定）8,520 件。辞書はインストール時に各マシンで生成され、パッケージにはビルドマシンの辞書を同梱する。
- 12-1（`48e2558`）: マージ順を `学習履歴 → ユーザー辞書 → システム辞書 → LLM` に変更（#13）。辞書が上限まであっても LLM の枠を確保し、LLM が辞書と重複して余った枠は辞書の残りで埋め戻す（#42 の 4）。マージ本体を純関数 `merge_candidate_lists` に切り出し。
- 12-2（`66cadcd`）: cost 帯（通常語 < 20000、記号 30000 + 行番号、絵文字 40000 + 行番号、`dict_schema` 2）で記号・絵文字を分け、`lookup_dict` / `lookup_symbols` / `lookup_emoji` に分割。マージ後に記号を上位 3 件の後ろに最大 15 個、残りは末尾、絵文字はその後に置く（mozc の SymbolRewriter に倣う）。builder は行順の cost、通常語だけの上限カット、`rakukan.dict.build.json` の出力。`install.ps1` は `dict_schema` が古い辞書を再生成、`build-installer.ps1` / `.iss` は json を同梱。記号帯と重複する通常語（「ほし → ★」）は通常語側を残す（mozc の cost は頻度情報に基づくため）。
- 12-3（`b80c493`）: `should_learn_and_log` を `learn_action`（Skip / Learn / LearnForce）に置き換え、候補ウィンドウから読みそのものを明示的に選んだ確定（候補送りで先頭以外、番号キー）だけ `learn_force` で学習（E-2）。E-3（同じ読みの並び）は現状維持（最終確定優先、頻度は日数換算のボーナス）。E-1 は main に予測が無く対象外。
- 実機確認（2026-09-12、辞書を再生成）: `みぎ`（右 みぎ ミギ → 記号 15 → 残り → 絵文字、6 ページ = 48 件）、`やじるし`（15 ページ = 130 件）、`ほし`（8 ページ = 69 件）、#13 の再現手順（登録直後は杜野が先頭、森野を選ぶと次回は森野が先頭）、`こうしょう`（辞書 50 件の読みで LLM 候補 4 件が 4 ページ目に入る）、`すごい` の明示選択で次回先頭。すべて合格。
- 途中で見つけた不具合（`a2b85d6`）: host 再起動直後の Space で「変換中...」のまま止まる。「モデル読み込み完了」の注入で ready になった後に `bg_start` を呼んでいなかった。ready 判定の後に `bg_start` する順序に変更し、実機確認済み。
- 追加で分かった問題点: 学習履歴を個別に削除する手段が無い（実機確認では `learn_history.bin` を退避して対応）。#37 に記載したのち、個別削除の手段は #46 に切り出して #37 をクローズした（2026-09-12）。
- #13 と #42 をクローズ、#37 に進捗を記載、#40 に順序入れ替えの完了を通知。#29（`priority = "low"`）は保留のまま。

### リリース 0.11.6（2026-09-12）

- Step 12 の 3 コミット（`48e2558` / `66cadcd` / `b80c493`）、PR #44（`df784ce` / `4d055ac`）、`a2b85d6` をまとめて 0.11.6 とした（`2963607`）。`docs/version-update-checklist.md` に従い `VERSION` / `Cargo.toml` / `Cargo.lock` / `rakukan_installer.iss` / WinUI `csproj` / `CHANGELOG.md` / `README.md` を更新し、タグ `0.11.6` を作成してプッシュした。
- 辞書の再生成が要るリリースのため、CHANGELOG と README に条件を明記した（インストーラーは再生成済みの辞書を同梱、`install.ps1` は辞書の隣の `rakukan.dict.build.json` を見て `dict_schema` が古い辞書を作り直す）。
- タグの状態: 0.11.4 は origin に存在する（`9bbab02`）が、ローカルには取得していない。ローカルにあるのは 0.11.0〜0.11.3 / 0.11.5 / 0.11.6。
- CHANGELOG に `[Unreleased]` は無い。次の開発を始める時点で作り直す。

### Step 13 調査（2026-09-13）

実装は行わず調査だけを実施した。**これまでの改修（0.11.5 / 0.11.6）の運用を見てから着手を判断する**方針で保留とする。

#### 確認した事実（コードとログで確認）

- host の起動回数は 1〜10 回/日（`rakukan-engine-host.log` の `rakukan-engine-host starting` を 2026-08-29〜09-13 で集計。ビルドとインストールを繰り返した 09-12 のみ 21 回）。起動ごとにモデルの BG 読み込みが入るため、その間の Space が未準備経路に落ちる。
- 未準備中の Space は `on_convert.rs` の `model_not_ready` 経路（`crates/rakukan-tsf/src/tsf/factory/on_convert.rs:1002` 以降）。辞書候補があれば辞書候補、無ければ読みそのものを候補にして「⏳ モデル読み込み中...」を表示するが、いずれも `stop_waiting_timer()` を呼び `llm_pending=false` で Selecting にする。つまり**再試行の経路が無く**、Esc で抜けて打ち直すまで LLM 変換が行われない。`a2b85d6` は「モデル読み込み完了の注入で ready になった瞬間」だけを直したもので、この経路は残っている。なお前の bg が走っていた場合は別分岐で、回収後にモデル未ロードなら `start_waiting_timer()` して Waiting に残す（同 1343〜1355 行、`result=model_not_ready_timer_fallback`）。ただしその先は次の項目の停止に落ちる。
- Waiting 側 `on_waiting_timer`（`crates/rakukan-tsf/src/tsf/candidate_window.rs:1191`）は、キー不一致で `bg_reclaim` + `bg_start` する分岐で `bg_start` が失敗すると `tracing::error!` を出すだけでタイマーを再武装しない（同 1488〜1494 行）。この場合 Waiting のまま止まる。
- 計測は `convert_timing` の `stage=` が 20 種類あり、区間ごとの `step_us` と結果行の `total_us` は既に出ている。無いのは RPC 単体の内訳（`rakukan-engine-rpc` の client 呼び出しに計測が無い）と、EditSession の要求から実行までの遅延。
- #43（推論失敗からの復帰）: host は推論失敗を検知しても自分では何もしない。TSF 側の `bg_timeout_watchdog`（`crates/rakukan-tsf/src/engine/state.rs:435`）は TSF プロセスごとの static で、PR #41 で見送った多重起動の問題と同じ構造を持つ。client 側には `host_spawn_guard`（`crates/rakukan-engine-rpc/src/client.rs:622` 以降、連続失敗でクールダウン）があり、backoff の土台にはなる。

#### 課題の整理（3 段階）

- **13-1 再試行（#39）**: `model_not_ready` で `llm_pending=true` のまま待機タイマーを起動し、`on_waiting_timer` が未準備なら `poll_model_ready` して、準備できた時点で 1 回だけ `bg_start` する。Waiting 側の再武装も合わせて直す。上限（例 10 秒）を超えたら `llm_pending` を降ろして文言を分ける。辞書が未準備の場合は対象外とするのが安全。
- **13-2 計測（#39）**: `call_with_retry` に所要時間（閾値超過は要求名つき WARN）、Convert 1 回あたりの RPC 回数と合計時間を結果行へ、EditSession の要求→実行の遅延、1 週間ぶんのログを集計する手順。
- **13-3 復帰設計（#43）**: 検知と抑止を host 側に置く（連続失敗を数えて exit し、次の呼び出しで spawn し直す）、直らない場合は起動時のマーカーで unrecoverable と判断、TSF は健全性を問い合わせる RPC で文言を決めるだけにする、watchdog も host 側へ移す、強制失敗のデバッグ設定で検証する。ABI / RPC に及ぶため設計合意を先に取る。
...
### Step 13 実装（2026-09-14）

13-1（再試行）・13-2（計測）・13-3（復帰、#43）を実装した。未リリース、実機確認は未実施。テスト 435 件成功、`cargo clippy --workspace --all-targets -- -D warnings` / `cargo fmt --all -- --check` クリーン。

#### 13-1 再試行（#39）

同日の辞書注入の修正で前提が変わった。ホストが全リクエストで注入を試みるようになったため「モデルは読み込み済みなのに注入されず ready にならない」状態が消え、13-1 は**「ready になったら変換をやり直す」だけ**になった。

採った方式（案 B）: `llm_pending` は立てず、`candidate_window` の thread_local に待機（読み・開始時刻・表示位置）を持つ。セッションは `Selecting { llm_pending: false }` のままなので、**待っている間も辞書候補の候補送りが通常どおり効く**。80ms の待機タイマーで `is_kanji_ready()` を確認し、ready になったら読みが変わっていないことを確かめて 1 回だけ `bg_start` → `llm_pending = true` で既存の更新経路へ合流する。上限 10 秒で打ち切り、文言を `⏳ モデル読み込みに時間がかかっています` に切り替える（候補表は残るので確定は続けられる）。Selecting を抜ける経路と `stop_waiting_timer` で待機は破棄する。

案 A（`llm_pending` を立てて待つ）は採らなかった。待機中の Space が `llm_pending` の分岐に入り、500ms のインライン待ちの後にフォールバックして再試行が止まるため。

**調査時の記述の訂正**: Step 13 調査（2026-09-13）に「`on_waiting_timer` は `bg_start` 失敗でタイマーを再武装しないので Waiting のまま止まる」と書いたが、これは誤り。`SetTimer` は periodic なので tick は 80ms ごとに鳴り続ける。実際の問題は「止まる」ではなく**上限が無く、進展しないまま回り続ける**こと（利用者から見て「⏳ 変換中...」のままなのは同じ）。そのため Waiting 側は再武装ではなく**上限 10 秒**を追加し、超えたら推論失敗と同じ経路で辞書候補へ落としてタイマーを止める実装にした。

#### 13-2 計測（#39）

- RPC 1 回ごとの所要時間を計測し、閾値 500ms 超で `rpc SLOW <要求名> elapsed_us=...` を WARN。意図して待つ要求（`ConvertSync` / `BgWaitMs` / `Create` / `Reload` / `Shutdown*`）は対象外（純関数 `rpc_slow_threshold_ms` + テスト）
- 区間の呼び出し回数・合計時間を `rpc_stats_reset` / `rpc_stats_snapshot` で提供し、`on_convert` の Drop ガード `ConvertRpcProbe` が `convert_rpc calls=N rpc_us=M total_us=T` を 1 行残す。分岐ごとに `return` する関数なので、結果行に足すのではなく Drop で確実に出す
- `EditSession` の要求 → `DoEditSession` 実行の遅延を計測し、100ms 超で WARN（生成箇所は `EditSession::new` の 1 か所）

#### 13-3 推論失敗からの復帰（#43）

判断と抑止をホストに置く。TSF DLL はアプリごとに別プロセスなので、TSF 側に再起動の判断を持たせると障害が続く間は各アプリが順に共有ホストを撃つ（0.9.13 の再起動ストームと同じ構造。PR #41 への Codex 指摘 P1）。

- **検知**: `dispatch_engine` の応答後に `bg_status()` を 1 回観測する。**状態が変わったときだけ**数えるので、TSF のポーリングで二重計上しない（`health::HealthTracker`、純粋ロジックでテスト 6 件）
- **段階**: 連続 3 回失敗 → ホストが自分で終了し、次の呼び出しでクライアントが新ホストを spawn（既存の透過再接続）。終了マーカーが 5 分以内で `attempt >= 2` の状態からまた 3 回失敗 → `unrecoverable`。推論が 1 回成功したら失敗もマーカーも捨てる
- **設計変更（3 段階 → 2 段階）**: 当初案の段階 1「ホストがプロセス内でモデルを作り直す」は現在の ABI では成立しない。`engine_start_load_model` は converter を持っていないときだけ働き（`ffi.rs:485-494`）、仮に新しい converter を作っても `engine_poll_model_ready` が「注入済み」として捨てる（`ffi.rs:550-557`）。差し替えには ABI 追加が要るため、ABI 据え置きの方針に従いプロセスごと作り直す形にした
- **マーカー**: `%LOCALAPPDATA%\rakukan\engine-recovery.txt` に `<exited_at_ms> <attempt>` の 1 行。serde_json を足さずに済ませるため JSON にしていない
- **状態の問い合わせ**: `EngineHealth`（`ok` / `recovering` / `unrecoverable`）を追加し、`PROTOCOL_VERSION` を 4 → 5（**ABI は 9 のまま**）。エンジンのロックを取らずに応答するので、変換中でも即答できる
- **文言**: TSF は状態を聞いて status 行を決めるだけ。`recovering` = 「⚠ 変換エンジンを再起動中…（GPU ドライバ更新後に必要）」、`unrecoverable` = 「⚠ GPU が使えません。Windows の再起動をお試しください」、それ以外は従来どおり「⚠ 変換エンジンが応答していません（辞書候補のみ）」（純関数 `status_for_health` + テスト）
- **検証手段**: `[diagnostics] force_inference_failure`（既定 false）で推論を必ず失敗させる。config JSON 経由なので ABI 変更なし。GPU デバイス消失は意図的に再現できないため、これが無いと段階を確認できない
- `bg_timeout_watchdog`（TSF 側、詰まり系）のホスト移設は今回に含めない。即時失敗系と詰まり系は別の状態機械で、同時に動かすと切り分けが難しくなるため

**注意**: `PROTOCOL_VERSION` を上げたので、古いホストが残ったままだと新しい TSF は接続できない（Hello でミスマッチ）。インストール手順のサインアウト／サインインで解消する。

### Step 13 の実機確認と、そこで見つけた 3 件（2026-09-14、0.11.7）

実機確認の過程で 3 件の不具合が出たため、いずれも修正して確認まで行った。

#### 1. 推論失敗が ABI の境界で `idle` に潰れていた

`engine_bg_status`（`ffi.rs`）の変換表に `"error"` の分岐が無く `_ => "idle"` に落ちていた。`conv_cache::status()` は `"error"` を返すのに、ABI を越えた時点で失敗の情報が消える。PR #41 の取り込み（`b657bb4`）で `conv_cache` 側に `"error"` を足したときの追加漏れ。

影響は 2 つ。**0.11.5 で直したことになっていた「推論失敗で Space が固まる」が実際には直っていなかった**（TSF の辞書候補フォールバックが一度も動かない）。そして Step 13-3 の復帰も、失敗を観測できないため動かない。

根拠: 全ログ（6 世代）で `falling back to dict candidates` が 0 件、同じ日の DLL ログには `conv-worker error` が 12 件。

修正と同時に、変換表を純関数 `bg_status_cstr` へ切り出し、`conv_cache::STATUS_VALUES`（状態名の全集合）を回して「ABI を越えても同じ文字列で出ること」を検証するテストを追加した。状態を増やして変換表を更新し忘れると落ちる。修正後は失敗を **113ms** で検知して辞書候補へ切り替わることを実機で確認（WARN 6 件）。

#### 2. モデル待ちの status 行が、辞書候補のある経路で出ていなかった

`model_not_ready` には「辞書候補あり」と「辞書候補なし（読みのみ）」の 2 経路があり、前者は status 行を出していなかった。利用者からは普通に変換できたように見え、10 秒後の打ち切り文言だけが唐突に出る。実機確認での指摘で判明。両経路で `⏳ モデル読み込み中...` を出すようにし、文言は定数へ切り出した。

#### 3. `recovering` の文言が利用者に届かなかった

健全性はホストのプロセスごとに持つため、自己終了した瞬間の `Recovering` はプロセスと一緒に消え、新しいホストは `Ok` から始まる。結果、再起動直後の変換は「⚠ 変換エンジンが応答していません」と表示され、「⚠ 変換エンジンを再起動中…（GPU ドライバ更新後に必要）」が出る窓は自己終了直前の約 50ms しかなかった。

マーカーから `prior_attempts > 0`（自己終了から戻った）と分かる間は `recovering` を返すようにした。推論が 1 回成功すれば `Action::Recovered` で `prior_attempts` が 0 に戻る。

#### 実機確認の結果

| 項目 | 結果 |
|---|---|
| 13-1 待機の起動・解除（Selecting を抜けたら破棄） | 確認 |
| 13-1 10 秒での打ち切りと文言の切り替え | 確認 |
| 13-1 status 行（辞書候補ありの経路） | 確認 |
| 13-2 `convert_rpc calls=N rpc_us=M total_us=T` | 確認 |
| 推論失敗の検知（113ms で辞書候補へ） | 確認 |
...
- モデル未準備の窓は 157ms しかなく手で狙えない。`model_variant` を存在しない値にすると確実に再現できる
- reload イベント（`Local\rakukan.engine.reload`）は **auto-reset で 1 プロセスにしか届かない**。全 TSF プロセスに config を反映させるには連続で Set する必要がある
- config を変えてもホストを作り直さないとモデルは切り替わらない（#49）。検証時はホストの終了も併せて行う
- `[diagnostics] force_inference_failure` は converter に焼き込まれるため、有効化にはホストの作り直しが要る

### リリース 0.11.7（2026-09-14）

`docs/version-update-checklist.md` に従い `VERSION` / `Cargo.toml` / `Cargo.lock` / `rakukan_installer.iss` / WinUI `csproj` / `CHANGELOG.md` / `README.md` を更新（`0065c50`）。ABI は 9、`dict_schema` は 2 のまま据え置き。内容は辞書注入の修正、推論失敗の検知と自動復帰、モデル待ちの再試行、Convert 区間の計測、DLL のログレベル追随、候補ウィンドウの `WS_EX_TOOLWINDOW`（PR #48）。

### PR #47 のマージと 0.11.8 を取り消し、#36 を再検討へ（2026-09-15）

PR #47（`input.text_field_mode`）をマージして 0.11.8 を作ったが、同日のレビューで 2 つの問題が出たため**マージとリリースを取り消した**（`7920e1f` で `a747f81` / `0cc5a4d` / `c298f9f` を revert）。機能そのものを否定するものではなく、設定の形と動作を再設計する。
...
- `[input]` の**下位テーブル**なので、この行より下に素のキーを書くと黙って下位テーブルに入り無視される
- キーにドットを含むため**引用符が必須**。外すと TOML はドット区切りキーとして解釈してパースに失敗し、起動時の経路（`ConfigManager::new` / `init_config_manager` の `unwrap_or_default()`）では**無言で全設定が既定値へ戻る**（IME 切替の経路だけは警告を出して前の設定を保持する、という非対称もある）
- キーが「適用範囲（アプリ名）」で**機能名の位置**に来ているため、設定を読んでも何の機能か分からない

**2. 動作が IME の作法に反する**

文字入力欄に入るたびに設定値で上書きするため、**利用者が自分で切ったモードが記憶されない**。英数を打つには毎回 IME をオフにする操作が必要になる。

rakukan には入力先ごとにモードを即時記憶する仕組みがある（`doc_mode_remember_current`、M1.7 T-MODE2）。したがって本来の不便は「同じレイヤーの再編集」ではなく、**新しい文字レイヤー（新しい入力先）が既定のオフで始まること**だった。毎回上書きする設計は、この記憶を無効化している。

#### 取り消しに伴う整理

- **0.11.8 は欠番**。GitHub にタグもリリースも作られていなかった（公開されているのは 0.11.7 まで）。ローカルタグ `0.11.8` は削除。次のリリースは **0.11.9**
- **#36 を再オープン**し、取り消しの理由を記載。設定の形と動作はこちらで引き取って検討する
- PR #47 に通知（作者は常用環境で使用中）
- 方式は `git revert`（GitHub の履歴は書き換えない）

#### 併せて記録する既存の課題

設定形式とは独立した問題で、別途対応する。

- 起動時のパース失敗が**無言で全設定を既定値へ戻す**（`reload_if_changed` は警告を出して前の設定を保持するのに対し非対称）
- config の再読込が**アプリごとにバラバラ**（reload イベントが auto-reset で 1 プロセスにしか届かない）

### アプリごとの IME 初期状態を config へ（#51、#36 完了、2026-09-15）

0.11.8 の取り消し後、設定の形と動作を引き取って設計し直し、`feat/ime-apps-config` ブランチで実装して PR #52 を main へマージした（`8940bb0` / `928e066`）。#36 はこれでクローズ。#51 は段 2（GUI）が残るため再オープン。

#### 経緯として確認したこと

- **ターミナル判定がコードに直書き**だった（`is_terminal_hwnd` がウィンドウクラス名 4 つと照合）。WezTerm や Ghostty のようにそれに該当しないターミナルは IME オフで始める手段が無かった
- 現状の記憶ロジックはログ上は正しく動いていた。`rakukan.log` 13.5MB で `doc_mode` のイベント 12,685 件、**保存値と復元値の一致 5,477 件・不一致 0 件**、復元値と違う値になった例 0 件。ただし手動変更の即時保存（`doc_mode_remember_current`）は `trace!` のためログに出ず、`DOC_MODE_STORE.try_lock()` の失敗は無言で抜ける（この 2 経路はログから確認できない）
...
ime_on_apps = ["msedge.exe", "chrome.exe", "firefox.exe"]
```

- **キーが機能、値が対象アプリ（exe 名）**。`[input]` 直下の素のキーなので、下位テーブルの位置問題（この行より下の素のキーが無視される）とキーの引用符の問題が起きない
- 判定は exe 名のみ。ウィンドウクラス名の直書き判定は削除し、**config を唯一の情報源**にした
- `ime_off_apps` の既定値は `serde(default)` にも持たせたので、キーが無い既存の設定でも従来どおりオフになる。`[]` で打ち消せる

#### 動作（アプリのアクティブ化ごとのセッション）

| 契機 | 動作 |
|---|---|
| アクティブになった | `ime_off_apps` は即オフ。`ime_on_apps` はまだ何もしない |
| （`ime_on_apps`）アプリ本体とは別の入力先に入った | オン（アクティブ化後 1 回だけ） |
| その後 IME を操作した | その状態を維持。新しい入力先に入っても引き継ぐ |
| インアクティブになった | 状態を破棄。次のアクティブ化で設定値から始まる |

- 対象アプリでは `dm_modes` / `hwnd_modes` の復元を通さない（「引き継がない」の実現）。対象外のアプリは従来どおり
- 「アプリ本体」は**アクティブ化後に最初にフォーカスされた入力先**とみなす。PR #47 はこれをプロセスの生存期間で持っていたため一度取り違えると終了まで直らなかったが、アクティブ化ごとにリセットされるので次のアクティブ化で直る
- 同じ exe が両方にある場合は、アクティブ化時にオフが適用されて `applied` が立つためオンは発動しない
- アクティブ / インアクティブは `ITfThreadFocusSink` の `OnSetThreadFocus` / `OnKillThreadFocus`（登録済みのものを利用）

#### 実機確認（2026-09-15）

- `ime_on_apps`: firefox.exe / msedge.exe / chrome.exe で入力欄に入ったときオン（18 回）。その後の手動変更が維持されることも確認
- `ime_off_apps`: wezterm-gui.exe（9 回）/ ghostty.exe（2 回）でアクティブ化時にオフ。実測した名前を既定値へ追加した
- 既定値のうち `conhost.exe` / `WindowsTerminal.exe` / `mintty.exe` は一致の記録が無く**未確認**
- **Photoshop は未確認**（環境が無い）。文字ツールは「本体とは別の入力先」に当たるため `ime_on_apps` で動くと考えているが確認していない

#### ログの方針

適用のログは info から debug へ下げた。アプリを切り替えるたびに出るため通常運用には要らず、実際のモード変更は `ime mode: X → Y` が info で記録している。

#### 残り

- **#51 段 2**: 設定アプリ（WinUI）での編集画面。exe 名を手で書かせずに実行中のアプリから選ばせたい。#33 でも設定アプリを触るので時期を調整する
- **#50**: 破棄済み DM ポインタが `dm_modes` に入り直す既存の課題は、今回の実装でも残る（対象外アプリの保存経路は従来どおり）

#### 段取りの反省

PR #52 の本文に `Closes #51 の段 1` と書いたため、マージ時に GitHub が #51 を自動クローズした。段階的な対応では `Closes` を使わない。

#### 今後の対応ステップ（2026-09-09 再構成）

Step 9 と Step 11 は完了、Step 10 以降が残っている。順序は変えず、各ステップに Issue を対応づけた。判断待ちの項目は時期未定。

| 順 | ステップ | Issue | 状態・判断待ち |
|---|---|---|---|
| 1 | Step 10 ローマ字入力状態の再構築 | #34、#38 | **完了（2026-09-12、0.11.5）**。#34 / #38 クローズ。`z` 系キー列の例外は #35 に残る |
| 2 | Step 12 学習順位と学習履歴 v2 | #37、#13、#42 | **完了（2026-09-12、0.11.6 でリリース）**。#13 / #42 / #37 クローズ。学習履歴の個別削除手段は #46 に切り出した。#29 は保留 |
| 3 | Step 13 低頻度改善と計測 | #39、#43 | **完了（2026-09-14、0.11.7 でリリース）**。13-1 再試行（上限 10 秒）/ 13-2 計測 / 13-3 復帰（2 段階、protocol v5）。実機確認で 3 件の不具合を修正（ABI で error が潰れる / status 行 / recovering がプロセスをまたげない）。`bg_timeout_watchdog` のホスト移設は含めず |
| 4 | Step 14 リリースと再計測 | なし | 対応するステップの実機確認後 |
| 5 | 10 月改修: インストール・更新・削除の再設計 | #33（PR #30 の方式を採用）、#46 | `docs/October_Install_Plan.md` の Step 1〜6。着手は Step 10〜13 の後。#33 は 2026-09-12 に範囲を広げ、インストール先を `%LOCALAPPDATA%` から `%ProgramFiles%` へ移す件、`install.ps1` の exe コピー失敗（`already exists`）、transcript 追記型ログの肥大化対策を追加した |
| 判断待ち | リーダー記号の入力方式 | #35 | 方式（Space 変換の候補 / `z` 系キー列）と文字の割り当てが未決。PR #31 は保留 |
| 完了 / 一部残り | 新しい入力欄の IME 状態（H） | #36、#51 | **#36 は完了（2026-09-15、PR #52 / `8940bb0` `928e066`、未リリース）**。`[input]` の `ime_on_apps` / `ime_off_apps` で設定する形にし、ターミナル判定の直書きを廃止。**#51 の段 2（設定アプリでの編集）が残る**。関連で #50 |
