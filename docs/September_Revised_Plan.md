...
- `[input]` 直下の素のキー。**キーが機能、値が対象アプリ**。下位テーブルの位置問題と引用符の問題が消える
- 適用は「**新しい入力先の既定値**」（ターミナル判定と同じ段）とし、記憶はそのまま働かせる。新しい文字レイヤーはオンで始まり、そこでオフにすればそのレイヤーでは記憶される
- `ime_off_apps` は、ウィンドウクラス判定（`CASCADIA_HOSTING_WINDOW_CLASS` / `ConsoleWindowClass`）に載らないターミナル（WezTerm 等）を補う用途
- 未決: 同じ exe が両方にある場合の優先、`ime_off_apps` をアプリ全体に適用するか新しい入力先に限るか

#### 取り消しに伴う整理

- **0.11.8 は欠番**。GitHub にタグもリリースも作られていなかった（公開されているのは 0.11.7 まで）。ローカルタグ `0.11.8` は削除。次のリリースは **0.11.9**
- **#36 を再オープン**し、取り消しの理由と再設計の方向を記載
...
| 判断待ち | 新しい入力欄の IME 状態（H） | #36 | PR #47 をマージして 0.11.8 を出したが、**設定の形と動作を理由に取り消した（2026-09-15）**。`[input]` 直下に `ime_on_apps` / `ime_off_apps` を置き、記憶を上書きしない形へ再設計する。未決は優先順位と `ime_off_apps` の適用範囲。関連で #50 |
