...
### PR #47 マージ・#36 クローズ・リリース 0.11.8（2026-09-15）

- **PR #47（#36 の H、`input.text_field_mode`）をマージ**（`c298f9f` / `0cc5a4d`）。#18 で出した条件（設定キー 1 個・既定 off・GUI 追加なし）に沿う実装。`config.toml` に exe 名で指定したアプリで、「文字入力欄が開いた」ときに指定モードで始める。判定は「プロセスで最初にフォーカスを得た DM を本体の文書とみなし、それ以外の初見の DM を文字入力欄とみなす」方式で、純関数 `text_field_mode_for` に切り出してテストされている。
- **レビューで確認した点**: 既定 off の経路（空 HashMap → `None` → 既存経路に触れない）、割り込み位置（前 DM のモード保存・`next_dm_ptr == 0` の早期 return・`dm_to_hwnd` 更新の後で、飛ぶのは `hwnd_modes` への書き込みだけ）、設定アプリの `SettingsStore.Save` が未知キーを保持するため設定が消えないこと。
- **作者の実機確認（Windows 11 / Photoshop）**: 編集開始で設定のモードになる・同じ文字レイヤーの再編集でも効く・設定を書かない状態は従来どおり、の 3 点は OK。**編集を終えてキャンバスへ戻ったときに元のモードへ戻すことはできず、これは方式の限界**（Photoshop は文字編集の終了時に `OnSetFocus` も `OnUninitDocumentMgr` も出さず、観測できる遷移は「本体 DM → 文字入力欄 DM」の一方向だけ）。PR 本文と #36 に限界として記載してクローズした。
- **Codex の指摘 2 件**: 1 件目（`Activate` 時の DM が基準になり機能が反転する）は実在したため作者が修正（`doc_mode_remove` で基準 DM を空にする）。2 件目（破棄済み DM ポインタの再利用）は `input.text_field_mode` の有無に関わらず起きる既存の挙動（`doc_mode_on_focus_change` の保存部が `prev_dm_ptr` を生存確認なしで `dm_modes` へ入れる）と判断し、**#50 として切り出して PR には含めなかった**。世代番号や tombstone を入れると全アプリのモード復元に影響するため。
- **0.11.8**（`docs/version-update-checklist.md` に従って更新）: 内容は PR #47 のみ。ABI 9 / `dict_schema` 2 は据え置き。
...
| 完了 | 新しい入力欄の IME 状態（H） | #36 | **PR #47 をマージして 0.11.8 でリリース（2026-09-15）**。#36 クローズ。編集終了時にモードを戻せない点は方式の限界として記載。関連で #50 を起票 |
