...
- **方針の更新**: #18 の 9/4 返信で示した A / G / H の条件は、その後の検討で方針が変わっている。A（文節変換）は #32 に切り出し、着手は 9 月計画と 10 月計画の後（`docs/Segment_Edit_Plan.md`）。G は打鍵中の自動畳み込みを採らない方向で #35 と `docs/Symbol_Leader_Input_Plan.md` で検討中（PR #31 は保留）。H は #36 で可否を判断する。9/4 の返信自体は訂正せず、PR が来た時点で個別に判断する。

#### 今後の対応ステップ（2026-09-09 再構成）

Step 9 と Step 11 は完了、Step 10 以降が残っている。順序は変えず、各ステップに Issue を対応づけた。時期を書いていない項目は判断待ち。

| 順 | ステップ | Issue | 状態・判断待ち |
|---|---|---|---|
| 1 | Step 10 ローマ字入力状態の再構築 | #34（本体）、#38（英単語の破壊。PR #26 は設計確定後に出し直し） | `docs/Step10_Romaji_Rebuild_Plan.md` §5 の 1・3・5・6 が判断待ち。TSF 記号経路の修正（#34 の問題点 4）は engine の 1 関数で済むため先行も可。`z` 系キー列の例外は #35 の判断に従う |
| 2 | Step 12 学習順位と学習履歴 v2 | #37、#13 | #13 は完了時にクローズ。完了後に #29（`priority = "low"`）を判断 |
| 3 | Step 13 低頻度改善と計測 | #39 | 再試行の安全条件（キースレッド非ブロック、retry storm 防止）を設計してから |
...
| 判断待ち | 候補・予測の品質（D） | #40 | 現行 main での再現確認が先。再現した項目だけ個別に切り出す |
| 判断待ち | かな run の辞書参照（Step 9 残件） | #16 | `DictLookup` API（#32 側）の転用可能性を含めて設計が必要 |
| 後回し | 文節変換 | #32 | 9 月・10 月は対応しない。`docs/Segment_Edit_Plan.md` |
