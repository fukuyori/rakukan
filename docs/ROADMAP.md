...
**v0.7.0 リリース済み（2026-04-24）**: 以下 4 件の user-facing bug fix を同梱。
...
- ✅ M1.8 T-MID1: Phase1B キューに `LIVE_CONV_GEN` + reading スナップショットを付与し stale 結果を discard（中間文字消失）
...
| --- | --- | --- | --- |
| **v0.7.0** ✅ 2026-04-24 | ✅ M1.5 T-BUG2（preview 長防壁）+ ✅ M1.7 T-MODE1（DM 破棄前退避）+ ✅ M1.7 T-MODE3（GA_ROOT 正規化）+ ✅ M1.8 T-MID1（Phase1B gen タグ）。**M1.6（host 再起動化）は v0.7.1 へ後送り** | minor | 尻切れ / ブラウザモード喪失 / 中間文字消失の即効対策をユーザへ（4 件全て bug fix） |
| **v0.7.1** | M1.6（host 再起動化、繰り延べ）+ M1.5 T-BUG1（早期 EOS 抑制）+ M1.7 T-MODE2（モード変更時即時保存）+ M1.8 T-MID2/3（EditSession stale check + SetText 排他）+ M1（基盤整理） | minor（host 再起動は user-facing） | host crash 根絶 + 変換品質改善 + ブラウザモード保持強化 + 中間消失の完全対策 + dead code 削減 |
| **v0.7.2** | M3（factory.rs 分割） | patch | 純リファクタ、動作不変 |
| **v0.7.3** | M2（ライブ変換可読性 + bg_peek/take + session_nonce） | minor | 機能追加含むため minor |
| **v0.7.4** | M4（LiveConvSession 集約） | minor | ライブ変換中枢の再設計 |
...
| **M1** | T3-A: engine accessor 整理 | 30 分 | 極小 | dead code 削除 | ✓ | v0.7.1 |
| **M1** | T3-B: dispose 系関数集約 | 1 時間 | 小 | 純粋な集約 | ✓ | v0.7.1 |
| **M1** | T1-D: docs / コメント cleanup | 1〜2 時間 | なし | 文書のみ | ✓ (M0 と並行可) | v0.7.1 |
| **M1 合計** | 基盤整理（低リスク先行） | **3〜5 日**（実作業 ≒ 4 時間、レビュー/実機含む） | 小 | リファクタ | — | v0.7.1 |
| **M1.5** | ✅ T-BUG2: preview 長サニティチェック | 半日 | 小 | bug fix (TSF 側防壁) | ✓ (M1.6 と並行可) | **v0.7.0 済** |
| **M1.5** | T-BUG1: 早期 EOS 抑制 / budget 拡大 | 1 日 | 中（LLM 品質影響要確認） | bug fix (engine 側本命) | ✓ | v0.7.1 |
| **M1.5 合計** | ライブ変換尻切れ修正 | **1〜2 日** | 中 | ★ user-facing bug | — | v0.7.0 + v0.7.1 |
| **M1.6** | T-HOST1: Request::Shutdown + restart 経路化 | 半日 | 小 | RPC 設計変更 | ✓ (M1.5 と並行可) | v0.7.1（繰り延べ） |
| **M1.6** | T-HOST2: 再起動時間計測 | 30 分 | なし | 計測のみ | ✓ | v0.7.1 |
| **M1.6** | T-HOST3: 段階表示 UI (10s/30s/60s) | 半日 | 小 | UI 追加 | ✓ | v0.7.1 |
| **M1.6** | T-HOST4: 読込中の入力握り潰し対策 | 半日 | 中（hot path 触る） | bug fix + UX | ✓ | v0.7.1 |
| **M1.6 合計** | 設定反映時の host 再起動化 | **2 日前後** | 小〜中 | ★ host crash 根絶 | — | **v0.7.1**（v0.7.0 から繰り延べ） |
| **M1.7** | ✅ T-MODE1: doc_mode_remove で破棄前に HWND へ退避 | 15 分 | 極小 | bug fix（最優先） | ✓ (M1.5/M1.6 と並行可) | **v0.7.0 済** |
| **M1.7** | T-MODE2: モード変更ごとに store を即時更新 | 半日 | 小（TL 追加 + 変更経路フック） | bug fix + 設計改善 | ✓ | v0.7.1 |
...
| **M1.7 合計** | ブラウザ入力モード保持 | **1 日前後**（T-MODE1〜3） | 小 | ★ user-facing bug | — | v0.7.0 + v0.7.1 |
| **M1.8** | ✅ T-MID1: Phase1B キューに gen タグを付与 | 半日 | 小〜中（race 再現確認含む） | bug fix（最優先） | ✓ (M1.5〜M1.7 と並行可) | **v0.7.0 済** |
| **M1.8** | T-MID2: EditSession 入口で composition stale check | 半日 | 小 | bug fix | ✓ | v0.7.1 |
| **M1.8** | T-MID3: Phase1A / Phase1B の SetText 二重適用排他化 | 半日 | 中（ロック/順序設計要） | bug fix | ✓ | v0.7.1 |
| **M1.8 合計** | ライブ変換中の中間文字消失 | **1〜2 日** | 中 | ★ user-facing bug | M2 と根本原因共有（先行バックポート） | v0.7.0 + v0.7.1 |
| **M2** | T1-B: on_live_timer 分解 | 半日 | 中（ロック順保持） | リファクタ | — | v0.7.3 |
| **M2** | bg_peek/take API 分離 (§18.3) | 1 日 | 中（API 変更 ~10 箇所） | リファクタ + 機能 | — | v0.7.3 |
| **M2** | session_nonce + gen (§18.3) | 1 日 | 中 | 機能追加 | — | v0.7.3 |
| **M2 合計** | ライブ変換ロジック可読性向上 | **3〜5 日** | 中 | リファクタ + 機能 | M1.6 後推奨 | v0.7.3 |
| **M3** | T1-A: factory.rs 分割 | **1〜2 日** | 小（純粋切り出し） | リファクタ | — | v0.7.2 |
| **M4** | T2: LiveConvSession 構造体集約 | **3〜5 日** | 中〜大（ライブ変換中枢） | リファクタ | 段階 PR 推奨 | v0.7.4 |
...
| カテゴリ | 総工数 | 備考 |
| --- | --- | --- |
| 必達（M1〜M4） | **12〜20 日** | M0 は暫定 PASS 済のため工数に含めない。M1.7 / M1.8 を含む |
| 必達のうち bug fix 枠（M1.5 + M1.6 + M1.7 + M1.8） | **5〜7 日** | user-facing + crash 対応、v0.7.0 + v0.7.1 で出す |
| 必達のうち refactor 枠（M1 + M2 + M3 + M4） | **7〜13 日** | v0.7.1〜v0.7.4 に分配 |
...
**目的**: `1234` → `一千二百三十四` のような位取り表記を候補に出し、文章中の数量表記を楽に入力できるようにする。金額用途の大字（壱弐参〜）もオプションで対応。
...
- 設定 `[input] digit_candidates_order = ["arabic", "fullwidth", "positional", "per_digit", "daiji"]` で候補順序を指定

**関連箇所（想定）**:

- `rakukan-engine` 内に `digit_kanji.rs`（仮）を新設し、`to_kanji_positional(u64)` / `to_kanji_daiji(u64)` を実装
- [digits.rs:76 `digit_candidates()`](../crates/rakukan-engine/src/digits.rs#L76) から呼び出し
- verify 対応: 漢数字 → 数値復元関数を書くか、「漢数字候補は生成元の数字 run を保持した信頼済みソース」として verify をスキップする分岐を入れる

**リスク**: 中（位取りルールの単体テストを丁寧に書く必要）
...
- `200` → `二〇〇` / `二百` が候補に出る
- `1234` → `一二三四` / `一千二百三十四` / `壱千弐百参拾四`（大字オン時）が候補に出る
...
2026-04-24: ✅ v0.7.0 リリース（bug fix 集中型）
            ...
            ✅ M1.7 T-MODE3 (GA_ROOT 正規化) ← T-MODE2 より前倒し
            ✅ M1.8 T-MID1 (Phase1B gen タグ + reading スナップショット)
...
```text
Week 2:   M1.6 (host 再起動化) ← v0.7.0 から繰り延べ、crash 根絶の本命
          M1.5 T-BUG1 (早期 EOS 抑制)
          M1.7 T-MODE2 (モード変更時即時保存)
          M1.8 T-MID2/3 (EditSession stale check + SetText 排他)
          M1 残（T3-A / T3-B）
          M1-D (docs cleanup) ← 隙間で消化
          │
          ├─────────── 🚀 v0.7.1 minor（M1.6 を含むため minor）
          ▼
Week 3:   M3 (factory.rs 分割)
          │
          ├─────────── 🚀 v0.7.2 patch
          ▼
Week 4-5: M2 (ライブ変換可読性)
          │
          ├─────────── 🚀 v0.7.3 minor
          ▼
Week 6-7: M4 (LiveConvSession 集約)
          │
          └─────────── 🚀 v0.7.4 minor
...
- **v0.7.0 を先頭に**: 尻切れ / ブラウザモード / 中間消失の 3 種 bug fix を最速でユーザへ。M1.6（host crash 根絶）は工数都合で v0.7.1 へ後送り
- **v0.7.1 で host crash + 変換品質**: M1.6 全体 + T-BUG1 を同梱。minor bump（M1.6 が user-facing）
- **v0.7.2 = 純リファクタ**: 動作変わらない PR として明示（blame/diff レビューを簡潔に）
- **v0.7.3 で機能追加**: bg_peek/take 分離が見えるため minor bump
- **v0.7.4 で中枢集約**: ライブ変換 API 形状の変化を包む

並行作業を許容すれば **v0.7.0 は達成（2026-04-24）、v0.7.4 まで +6 週** で到達見込み。個人開発の余裕状況で各リリース間隔は伸縮する。**crash が再発しない限り M5 は開封しない** ため、残総工数は 10〜17 日（v0.7.0 分 ~1 日消化済）で上限が決まる。
