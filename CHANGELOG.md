...
- **エンジンホストが入れ替わると、辞書・ユーザー辞書・学習履歴が黙って無効になる問題を修正**: GPU ドライバ更新やホストの異常終了などでホストのプロセスだけが入れ替わると、変換は動くのに辞書候補が 1 件も出ず、確定しても学習されない状態で動き続けていた（辞書は読み込み済みなのにエンジンへ渡っていなかった）。注入をホスト側で必ず行うようにしたため、ホストがどんな理由で入れ替わっても辞書が有効になる。トレイの「エンジン再起動」からも復旧できるようにした。辞書が無い状態で確定したときはログに警告を残す。
- **推論が失敗したときに「⏳ 変換中...」のまま止まる問題を修正**（[Issue #43](https://github.com/fukuyori/rakukan/issues/43)）: GPU ドライバの更新・スリープ復帰・TDR で推論が即座に失敗する状態になると、失敗が ABI の境界で「何も起きていない」扱いに潰れ、待ち続けていた。0.11.5 で入れた「固まりの解消」が実際には働いていなかった。失敗を検知して辞書候補へ切り替えるようになった（実測 113ms）。
- **候補ウィンドウがタスクバーと Alt+Tab の一覧に出る問題を修正**（[PR #48](https://github.com/fukuyori/rakukan/pull/48)、nick20002005）: 押しても何も起きないタスクバーのボタンと、無題の Alt+Tab 項目が消える。

### Added

- **推論が失敗し続けたときにエンジンが自分で復帰する**（Issue #43）: 連続 3 回失敗するとホストが自分で終了し、次の変換で新しいホストが起動する（GPU デバイスを取り直す）。それでも直らない場合は「⚠ GPU が使えません。Windows の再起動をお試しください」と表示して辞書候補のみで動作を続ける。復帰中は「⚠ 変換エンジンを再起動中…（GPU ドライバ更新後に必要）」と表示する。判断はホスト側で行うため、アプリを何個開いていてもエンジンの再起動が重複しない。
- **モデル読み込み中に変換したとき、読み込み完了後に自動で変換をやり直す**（[Issue #39](https://github.com/fukuyori/rakukan/issues/39)）: 従来は Esc で抜けて打ち直すまで LLM 変換が行われなかった。待っている間も辞書候補の候補送りは使え、10 秒を超えると「⏳ モデル読み込みに時間がかかっています」に切り替えて待機をやめる。「⏳ モデル読み込み中...」は辞書候補が出ている場合にも表示するようにした。
- **エンジン DLL のログレベルが `config.toml` の `log_level` に追随する**: 従来は環境変数 `RAKUKAN_LOG`（既定 info）だけで決まり、`log_level` を上げても候補の内訳などが記録されなかった。`RAKUKAN_LOG` を明示している場合はそちらを優先する。
- **変換 1 回あたりの RPC 回数と所要時間、編集権の取得遅延をログに残す**（Issue #39）: 「遅い」「止まる」の報告を区間ごとに切り分けられるようにした。閾値を超えた RPC と編集権の取得は警告として残る。
- **診断用の設定 `[diagnostics] force_inference_failure`**（既定 `false`）: 推論を必ず失敗させる。GPU デバイス消失は意図的に再現できないため、上記の復帰動作を確認するために使う。

## [0.11.6] - 2026-09-12

### Changed

- **候補の順位を「学習履歴 → ユーザー辞書 → システム辞書 → LLM」にした**（[Issue #13](https://github.com/fukuyori/rakukan/issues/13) / [Issue #37](https://github.com/fukuyori/rakukan/issues/37)）: 従来はユーザー辞書が学習履歴より上にあり、登録した表記を別の候補で上書きできなかった。登録直後はこれまでどおりユーザー辞書の表記が先頭で、候補から別の表記を選んで確定したときだけ学習がそれを上書きする。使わなくなれば学習スコアの減衰で元に戻る。
- **記号・絵文字を通常語の後ろに並べ、mozc の優先順で差し込むようにした**（[Issue #42](https://github.com/fukuyori/rakukan/issues/42)）: 「みぎ」で矢印記号 40 個が「右」より前に並び、候補上限で「右」が切り落とされていた。辞書の記号・絵文字を通常語と別の cost 帯に置き、通常候補の上位 3 件の後ろに記号を最大 15 個、残りの記号は末尾、絵文字はその後ろに置く。記号の並びは mozc の symbol.tsv の行順になり、1 読みあたり 50 件の上限は通常語だけに適用されるため「やじるし」の記号が途中で切れなくなった。**辞書の再生成が要る**: インストーラーは再生成済みの辞書を同梱する。`install.ps1` は辞書の隣の `rakukan.dict.build.json` を見て古い辞書を自動で作り直す。
- **候補ウィンドウから読みそのもの（ひらがな）を明示的に選んで確定すると学習するようにした**（Issue #37）: 候補送りで先頭以外の読みを選ぶか番号キーで選んだ場合だけ学習し、次回はその読みが先頭に来る。Enter でそのまま確定した場合や先頭候補が読みだった場合は従来どおり学習しない。

### Fixed

- **ひらがな入力中に打った英単語が後ろの日本語ごと壊れる問題を修正**（[Issue #38](https://github.com/fukuyori/rakukan/issues/38) / [PR #44](https://github.com/fukuyori/rakukan/pull/44)、nick20002005）: `seedreamのペースはどう` が `seedreamnoぺーすはどう` になっていた。打鍵ログから読みの先頭の英単語を復元して変換器に渡す。復元するのは「素の ASCII の直後が助詞」で「読みの先頭にある」場合に限り、`google`（右端が決まらない）や `これはseedreamの…`（日本語の前置き）は復元しない。助詞を含まない前置き（`きょうseedreamの…`）は英単語に巻き込まれる既知の限界がある（正常な日本語の変換は壊さない）。
- **辞書候補が上限（40 件）まである読みで LLM 候補が 1 件も入らなかった問題を修正**（Issue #42）: LLM の新規候補のぶん枠を残して辞書候補を積むようにした。
- **エンジンホストの再起動直後に Space を押すと「⏳ 変換中...」のまま止まる問題を修正**: モデル読み込み完了の通知を受けて準備完了に切り替わった後に変換を開始していなかった。

## [0.11.5] - 2026-09-12

### Fixed

- **誤入力を Backspace で消した後、残ったローマ字が次の文字と結合しない問題を修正**（[Issue #34](https://github.com/fukuyori/rakukan/issues/34)）: `kt` → Backspace → `a` が「kあ」になり、`nt` / `tt` では「ん」「っ」が残っていた。入力ログを打鍵・出力・種別を持つ構造に改め、未確定ローマ字がある状態の Backspace は末尾のローマ字区間を打鍵列から作り直すようにした（`kt` → Backspace → `a` = か、`nt` → な、`tt` → た、`kanakq` → Backspace → `a` = かなか）。
- **記号・Shift+英字・数字が未確定ローマ字を追い越す問題を修正**（Issue #34）: `k` + `.` が「。k」、`n` + `.` が「。n」、`k` + Shift+`A` が「Ａk」になっていた。これらの入力では先に未確定ローマ字を閉じてから追記する（「k。」「ん。」「kＡ」）。未確定ローマ字がある状態の数字も `digit_width` に従うようになった。
- **F9 / F10 → F6 の復元で Shift+英字・数字・末尾の「ん」が壊れる問題を修正**（Issue #34）: 「るーむＡしつ」が「るーむあしつ」に、`kon` が「こn」に戻っていた。復元は入力時の出力をそのまま連結するようにした。
- **「きゃ」から「ゃ」だけ消した後に F9 の綴りが復元できない問題を修正**（Issue #34）: 表示 1 文字を消したとき、入力ログを残った表示に合う綴りに書き換える（`kya` → `ki`、`sha` → `shi`、`tta` → `tt`、`wi` → `u`。元の綴りと共有する接頭辞が長いものを優先し、残るかなが母音なら母音字 1 文字にする）。
- **Space 変換で末尾の未確定ローマ字が消える問題を修正**（Issue #34）: 「た」+ `t` で Space を押すと候補と確定から `t` が消え、学習キーに `t` が混ざっていた。未確定ローマ字は候補の後ろに接尾辞として残し（`alpha_width` に従って全角 / 半角）、確定に含め、学習キーには含めない。Backspace / Esc で候補選択をやめると Space 前の状態（「たt」）に戻り、続けて母音を打てば結合する。
- **推論が失敗すると Space が固まる問題を修正**（[PR #41](https://github.com/fukuyori/rakukan/pull/41) の一部を取り込み）: GPU ドライバ更新やスリープ復帰などで推論が即座に失敗する状態になると、候補ウィンドウに「⏳ 変換中...」が出たまま候補送りも確定もできなくなっていた。推論の失敗を「候補なし」と区別し、待つのをやめる経路で辞書候補に切り替えて「⚠ 変換エンジンが応答していません（辞書候補のみ）」を表示する。エンジンの自動復帰は含まない（別途検討）。

## [0.11.4] - 2026-09-07

### Changed

- **入力状態を「IME オン / IME オフ」の 2 値に統一した**: 従来の入力モード 3 値（ひらがな / カタカナ / 英数）を廃止し、ひらがな = IME オン（かな漢字変換）、英数 = IME オフ（直接入力）とした。カタカナモードは表示だけが変わるひらがなモードで入力挙動を変えていなかったため廃止し、カタカナは `F7` / 無変換キーの `cycle_kana` で入力する。言語バーのメニューは「IME オン / IME オフ」の 2 項目になり、アイコンは「あ」/「A」。
- **IME 切替のすべての経路を 1 か所に集約した**（`tsf/ime_sync`）: キー操作、言語バーメニュー、フォーカス移動時の復元、Activate 時の初期化、外部アプリによるコンパートメント変更の取り込みが同じ関数を通る。内部状態を唯一の正とし、TSF の `KEYBOARD_OPENCLOSE` はそこから導出して書くだけにした。表示もキー処理もコンパートメントを読まない。
- **keymap のアクション名を整理した**: IME 制御は `ime_toggle` / `ime_on` / `ime_off` の 3 つ。旧名は互換のため別名として読み込む（`mode_hiragana` → `ime_on`、`mode_alphanumeric` → `ime_off`、`mode_katakana` → `katakana`（F7 変換））。プリセットも同じ対応で更新した。
- **`config.toml` の `[input] default_mode` の値を `"on"` / `"off"` にした**: 旧値 `"hiragana"` / `"alphanumeric"` も引き続き受け付ける。`remember_last_kana_mode` のキー名は変えていない。
- **設定アプリの「主要ショートカット」を再構成した**: IME 切替 / IME ON / IME OFF / 変換開始 / ひらがな確定 / 取消 / 全取消 の 7 行。旧アクション名は読み込み時に新名称へ写し、保存時は新名称で書き出す。「初期入力モード」は「起動時の IME 状態」（off / on）に、記憶トグルのラベルは「前回の IME オン/オフをアプリごとに記憶する」に変更。

### Added

- **修飾キーの左右を区別できるようにした**: `LCtrl` / `RCtrl` / `LShift` / `RShift` / `LAlt` / `RAlt` をキー名に追加（例: `RAlt+Caps`）。従来の `Ctrl` / `Shift` / `Alt` は左右どちらでも一致する。左右指定と汎用指定の両方があるときは左右指定を優先する。
- **設定アプリの「主要ショートカット」にヘルプ表示を追加**: 「キー名の一覧と注意」ボタンで、使えるキー名の一覧と、割り当てできないキー・効かない組み合わせなどの注意事項を表示する。保存時のキー名エラーからもこの一覧を参照するよう案内する。
- **設定アプリを単一インスタンスにした**: 2 つ目を起動すると新しいウインドウを開かず、既存のウインドウを前面に出す（最小化中は復元）。

### Fixed

- **言語バーの表示と実際の入力状態がずれる問題を修正**: 言語バーメニューで英数（IME オフ）にした後にショートカットでひらがな / カタカナへ切り替えると、内部状態は変わってかな入力になるのに表示が「A」のまま残っていた。ショートカット経路がコンパートメントを更新せず、表示だけがコンパートメントを優先して読んでいたのが原因。上記の経路集約で解消した。
- **IME オフ操作で候補選択中の表示文字列が確定されず、候補ウィンドウが残る問題を修正**（[PR #24](https://github.com/fukuyori/rakukan/pull/24) の指摘を取り込み）: 半角/全角キーだけでなく、`ime_off` キーと言語バーメニューからの切替でも、画面に出ている文字列（候補選択中・ブロック選択中・範囲選択中・待機中）を確定し、候補ウィンドウを閉じる。

## [0.11.3] - 2026-09-03

### Added

- **候補ウィンドウのフォントサイズを変更できるようにした**（[Issue #3](https://github.com/fukuyori/rakukan/issues/3) / [PR #5](https://github.com/fukuyori/rakukan/pull/5)）: `config.toml` の `[appearance]` `candidate_font_height`（10〜72px、既定 17）で指定する。行の高さ・余白・最小幅も同じ比率で拡大されるため、この 1 項目だけでレイアウトが崩れずにスケールする。レイアウトは表示ごとに固定され、作業領域に収まらない場合は収まる最大のフォントサイズへ自動縮小（下限 9px）、幅と表示位置も作業領域内にクランプされる。
- **設定アプリの「候補表示」にフォントサイズ項目を追加**: GUI から 10〜72 の範囲で変更・保存できる。

### Fixed

- **設定の保存が一部のアプリの IME に反映されないことがある問題を修正**: 保存通知の名前付きイベントは auto-reset で、複数アプリで IME を使っていると 1 プロセスにしか届かなかった。候補ウィンドウの表示直前に `config.toml` の更新を確認し、表示設定を各プロセスが自力で拾うようにした（`config.toml` を手で編集した場合も次回の候補表示から反映される）。
- **設定アプリ: 保存後の「反映しました」表示が残り続ける問題を修正**: 設定を変更した時点で表示を閉じ、次の保存の結果が新しく表示されたことが分かるようにした。

## [0.11.2] - 2026-09-03

### Fixed

- **数字とかなが混在する読みで辞書がバイパスされ、変換品質が落ちる問題を修正**（[Issue #6](https://github.com/fukuyori/rakukan/issues/6) / [PR #7](https://github.com/fukuyori/rakukan/pull/7)）: 「5まん」が「5満」「5マン」になり「5万」が候補に一切出なかった。数字保存の検証が「万」を数値 10000 として数え、正しい候補を数字改変とみなして捨てていたのが原因。数字直後の位取り数詞（十/百/千/万/億/兆、連濁形の「ぜん」「びゃく」「ぴゃく」を含む）を数字に付いた単位として扱うよう境界判定を修正し、数詞候補は通常の候補経路（重複排除・順位付け・件数制限・生読み fallback）を通すようにした。「5まんえん」「だい5まん」「3.5まん」にも効き、`2024ねん` → `二千二十四年` などの既存の数値表現変換は変わらない。単位の読み飛ばしは入力読みに対応する数詞かながある場合に限るため、「5えん」→「5万円」のような単位の挿入（数の改変）は引き続き拒否する。かな run が辞書候補を参照できない構造課題は [Issue #16](https://github.com/fukuyori/rakukan/issues/16) で追跡する。
...
- **変換済みカタカナ語を含む文が context から捨てられる問題を修正**（8月ログで約 1,700 回/月）: echo strip（未変換ひらがな確定文の除去）が読みのカタカナ形でも照合していたため、「インストーラ」「ドキュメント」「トランプは」のような正当に変換済みのカタカナ語を含む文まで context から除去し、変換品質を下げていた。判定をひらがな needle のみに限定した。F7 等でカタカナ確定した文も除去しなくなるが、カタカナ echo は出力として正しく、途切れた断片候補は候補側で棄却される。
...
- **入力モード切替のたびに keymap.toml を同期で読み直していたキーストールを解消**（8月ログで SLOW OnKeyDown の直前に `keymap loaded` が 53 回、最悪 931ms）: モード切替時は `keymap.toml` の mtime が変わった場合だけ読み直す（変化が無ければ `metadata` 1 回のみ）。更新・作成は新しい keymap、削除は既定 keymap、parse 失敗は直前の keymap を維持し、壊れたファイルを切替ごとに parse し直さない。Activate 時の読み込みは従来どおり必ず行う。
