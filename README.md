...
v0.9.8 では以下の変更が入りました。

- **記号入力後のライブ変換再開**: ライブ変換中に `、` `。` などの区読点を入力した後、続けてひらがなを入力するとライブ変換が再開するようになった。例：「きょうは、」と入力してさらに「またあした」と続けると「今日は、また明日」のようにライブ変換が動作する。区読点のみで終わっている場合は引き続き変換しない。
