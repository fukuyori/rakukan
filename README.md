...
v0.10.2 では以下の変更が入りました（7月運用ログ分析にもとづく改善の第 3 弾。計画は `docs/JULY_LOG_IMPROVEMENT_PLAN.md`）。

- **確定テキスト消失を修正**: アプリ側が一時的に読み取り専用（TS_E_READONLY）のとき、確定テキストが丸ごと消えることがあった。再試行と「表示中テキストのままの確定」で入力を保全する。
- **無駄なバックグラウンド変換を削減**: 読みの末尾が未確定ローマ字（例: `ぁr`）の間はライブ変換を起動しない。
