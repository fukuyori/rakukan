...
v0.11.5 はローマ字入力の状態を打鍵ログから再構築するリリースです（Issue #34）。誤入力を Backspace で消した後に残ったローマ字が次の文字と結合しない問題（`kt` → Backspace → `a` が「kあ」）、記号・Shift+英字・数字が未確定ローマ字を追い越す問題（`k` + `.` が「。k」）、F9 / F10 → F6 の復元で Shift+英字や末尾の「ん」が壊れる問題、「きゃ」から「ゃ」だけ消した後に F9 の綴りが復元できない問題、Space 変換で末尾の未確定ローマ字が消える問題を修正しました。あわせて、GPU ドライバ更新などで推論が失敗すると Space が固まる問題を修正しました（PR #41 の一部を取り込み。エンジンの自動復帰は別途検討）。
