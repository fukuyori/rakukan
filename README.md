...
v0.9.5 では以下の変更が入りました。

- **ユーザー辞書エディターの複数候補表示バグ修正**: 複数の変換候補を持つエントリを編集ダイアログで開くと 1 番目しか表示されなかった問題を修正。WinUI 3 `TextBox` の初期化順序に起因するバグ（`AcceptsReturn` 設定前に `Text` を設定すると改行が除去される）を修正。

v0.9.4 の主な変更については [CHANGELOG.md](CHANGELOG.md) を参照してください。
