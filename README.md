...
v0.9.9 では以下の変更が入りました。

- **ユーザー辞書候補のライブ変換反映**: `かっことじ` などのユーザー辞書候補を、ライブ変換中にも読みを明示してマージするようにした。
- **記号入力と長文ライブ変換の安定化**: 未入力状態の記号入力を未変換状態で保持し、長い入力でライブ変換結果が前回より急に短くなった場合は直前 preview を使って表示の消失を抑えるようにした。
