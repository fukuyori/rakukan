...
v0.10.1 では以下の変更が入りました。

- **echo strip（context 汚染対策）の誤爆を削減**: 変換済みの文への偶然一致で正当な文脈まで切り捨てていた問題を修正した。エコー源の判定を「8 文字以上のかな連続 run」に絞り、除去も該当文のみに限定。あわせて、ひらがなのみの確定文は最初から文脈に入れない根本対策を追加した。
- v0.10.0: **エンジン（LLM モデル）の二重ロード乱発を修正**（7月の運用ログで月 800 回発生）。モデルのロードは初回起動時と設定変更時のみになった。
