...
v0.10.3 では **jinen-v2 モデル（Qwen3 ベース）** を追加しました。`config.toml` の `model_variant` を `jinen-v2-xsmall-q5`（約 28 MB）/ `jinen-v2-small-q5`（約 81 MB）などに書き換えるだけで切り替えられます（f16 variant もあり。デフォルトは v1 のまま）。
