...
v0.9.13 では以下の変更が入りました。

- **変換停止の主因対策**: true beam search を 1 コンテキスト + batched decode 方式に書き換え、大幅に高速化した。beam 経路にも 15 秒のウォールクロックタイムアウトを追加。
- **途中切れ・重複出力の対策**: EOS 未到達の未完了ビームを候補にせず、候補長の上限安全網とタンデム反復検出で退化出力を棄却するようにした。
- **表示と確定テキストの不一致を修正**: ひらがな表示のまま Enter すると未表示の変換結果が確定される問題を修正し、確定は表示済みテキストのみ（WYSIWYG）とした。
- **engine-host の多重起動防止と診断ログ強化**: named mutex によるシングルトン化、エンジン DLL 内 tracing ログ、変換ごとの観測ログを追加。
