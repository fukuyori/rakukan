...
v0.9.10 では以下の変更が入りました。

- **短い読みの候補更新**: 短い読みで即時辞書候補を仮表示した場合も `llm_pending` を維持し、LLM 完了後に辞書候補と LLM 候補を後追いマージするようにした。
- **記号追加時の表示同期**: `あ、` のように記号を追加したとき、engine の `preedit_display()` を表示に使い、古い session preedit による表示ズレを防ぐようにした。
