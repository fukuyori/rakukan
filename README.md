...
v0.9.11 では以下の変更が入りました。

- **ユーザー辞書編集の即時反映**: 設定画面や外部エディタで `user_dict.toml` を編集した後、engine 再起動に依存せず次回候補参照時にユーザー辞書だけを hot reload するようにした。
- **設定変更後の engine 再生成**: engine-host が既存 engine の `config_json` を保持し、次回接続時に設定差分があれば engine を作り直すようにした。
