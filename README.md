...
v0.10.0 では以下の変更が入りました。

- **エンジン（LLM モデル）の二重ロード乱発を修正**: BG 変換中やフォーカス切替のたびにモデルを丸ごとロードし直すことがあった問題を修正した（7月の運用ログで月 800 回発生）。モデルのロードは初回起動時と設定変更時のみになり、ロード中の「engine busy / not ready」も解消される。
