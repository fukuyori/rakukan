...
v0.9.4 では以下の変更が入りました。

- **区読点分割変換の対象記号を拡張**: これまで `、` `。` `！` `？` のみだった区切り記号を、全角記号（`（）～｛｝；：＠＃` 等）・ASCII 印字記号（`@#()~` 等）・和文記号（`「」・`）まで拡大。読みに記号が含まれると自動的にブロック分割変換へ移行する。また、区切り記号を含む読みではライブ変換バックグラウンドを起動しない。
- **候補順序の変更**: 辞書候補（mozc）を LLM 候補より先に表示するよう優先順位を変更（user_dict → 学習 → mozc_dict → LLM）。
- **ライブ変換開始文字数の設定化**: `config.toml` の `[live_conversion] min_chars`（デフォルト 3）と WinUI 設定 UI で調整可能になった。
- **言語バーの読込中インジケーター**: エンジン未準備中は言語バーのアイコンに「ー」を表示し、変換可能になると「あ」/「ア」に自動更新。
