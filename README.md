...
- `ime_off_apps` に挙げた exe は、アクティブになるたび IME オフで始まります（既定はターミナル 5 種）。`ime_on_apps` に挙げた exe は、アプリ本体とは別の入力先に入ったとき IME をオンにします。どちらも操作中に IME を変えればその状態が続き、インアクティブになると捨てられます（次にアクティブにすると設定値から始まります）
