...
v0.9.7 では以下の変更が入りました。

- **LLM 候補の学習対応**: 候補ウィンドウから明示的に選択した LLM 候補を学習履歴に記録するようにした。これまで MOZC 辞書に存在しない CJK surface は辞書ガードにより学習されなかったが、候補ウィンドウからの明示選択時はガードをバイパスして記録する。LiveConv の Enter 自動確定は従来通りガードあり。30 日半減期の自然減衰も引き続き機能する。
