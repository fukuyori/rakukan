...
- **長文高速入力時の後方欠落を抑制**: LiveConv 継続入力で合成表示が canonical reading に対して明らかに短い場合、当該キー入力では完全なひらがな preedit 表示へ戻すガードを追加しました。
- **高速寄りの標準設定へ調整**: 未指定時の候補数を 6、ライブ変換 beam を 1、Space 変換 beam を 6 に寄せ、ライブ変換の速度と候補表の幅を両立する標準設定にしました。
- **Space 変換 beam を WinUI 設定に追加**: `conversion.beam_size` を GUI から調整できるようにしました。
- **候補数と beam の食い違いを防止**: WinUI 設定で候補数を上げた場合、`conversion.beam_size` も同じ値まで自動で追従します。
- **設定画面を WinUI に一本化**: 旧 Win32 設定画面を削除し、配布される設定 UI は WinUI 版のみになりました。
- **候補表示数の反映を改善**: Space 変換で候補が 1 件足りない場合は元の読みを退避候補として表示します。1画面の表示は従来どおり最大 9 件です。
...
- **ログローテーションを追加**: TSF / engine-host のログが肥大化し続けないよう、起動時に 16 MiB 超のログを最大 5 世代までローテーションするようにしました。
- **azooKey 型の候補メタデータを導入**: TSF 内で `CandidateView` を持ち、候補表と本文 composition が同じ候補レコードを参照する土台を追加しました。
- **候補表示の相関ログを追加**: `candidate_display_probe` で LiveConv preview / Space 初期候補 / pending update の候補対応を追跡できるようにしました。

## 0.8.7 変更点

- **Space 候補表示の初動を改善**: LiveConv 中に Space を押した場合、Space 押下時点の preview を候補表の第1候補として表示し、本文 composition も同じ候補に揃えるようにしました。
- **候補表示状態を整理**: 通常 Space 経路で `SessionState::Selecting` の snapshot から候補表と本文表示を作るようにし、ハイライト行と本文候補がずれにくくしました。
- **長文ライブ変換の表示安定性を改善**: 生成側で finished beam を優先し、途中切れ preview が入力継続時に後方欠落として継承されるリスクを下げました。

## 0.8.6 変更点

- **ライブ変換の開始条件を調整**: ライブ変換 preview は、読みが 3 文字以上になってから BG 変換と timer preview を起動するようにしました。
- 1〜2 文字の入力中はプリエディット表示を維持し、Space 変換 / 確定経路は従来どおり個別に処理します。
...
- **大字候補を追加**: 数字だけの reading で `1234 → 壱千弐百参拾四`、`10000 → 壱万` のような大字候補を表示します。
- **数字候補の順序設定を追加**: `[input] digit_candidates_order = ["arabic", "fullwidth", "positional", "per_digit", "daiji"]` で候補種別と表示順を指定できます。指定から外した種別は候補に出ません。
  - `arabic`: 半角数字、`fullwidth`: 全角数字、`positional`: 位取り漢数字、`per_digit`: 桁並び漢数字、`daiji`: 大字

## 0.8.3 変更点

- **数字間の区切り文字自動変換を追加**: 数字直後に `、` / `。` を入力した場合、数値表記として `,` / `.` をプリエディットに追加します。例: `2、4` → `2,4`、`2。5` → `2.5`。
- `[input] digit_separator_auto = true` を追加しました。デフォルトは `true` です。

## 0.8.2 変更点

- **位取り漢数字候補を追加**: 数字だけの reading で、既存の半角 / 全角 / 桁並び漢数字に加えて `1234 → 千二百三十四`、`10000 → 一万` のような位取り漢数字候補を表示します。
- カンマ・小数付き数値も通常漢数字候補に対応しました。例: `2,400 → 二千四百`、`2.5 → 二点五`。
- 大字候補と候補順設定は 0.8.4 で追加しました。

## 0.8.1 変更点

- **記号の半角 / 全角候補を追加**: `USB-C`、`A+B`、`(test)` のような ASCII 記号を含む reading で、記号部分も半角 / 全角候補として扱います。
- 数字・アルファベット・記号の混在 run を LLM に渡さず literal として保護し、既存の候補合成経路で `USB-C` / `USB－C` / `ＵＳＢ-Ｃ` などを提示できるようにしました。
